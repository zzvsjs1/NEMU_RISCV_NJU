...
static int64_t file_size(SDL_RWops *rw)
{
  ...
  if (old < 0) return -1;
  if (fseek(rw->fp, 0, SEEK_END) != 0) return -1;
  long end = ftell(rw->fp);
  if (end < 0) return -1;
  if (fseek(rw->fp, old, SEEK_SET) != 0) return -1;
  return end;
}

static int64_t file_seek(SDL_RWops *rw, int64_t offset, int whence)
{
  if (fseek(rw->fp, (long)offset, whence) != 0) return -1;
  return ftell(rw->fp);
}
...
{
  int ret = 0;
  if (rw->fp != NULL) ret = fclose(rw->fp);
  free(rw);
  return ret;
}
...
static int64_t mem_seek(SDL_RWops *rw, int64_t offset, int whence)
{
  ...
  switch (whence) {
    case RW_SEEK_SET: next = offset; break;
    case RW_SEEK_CUR: next = rw->mem.offset + offset; break;
    case RW_SEEK_END: next = rw->mem.size + offset; break;
    default: return -1;
  }

  if (next < 0 || next > rw->mem.size) return -1;
  rw->mem.offset = next;
