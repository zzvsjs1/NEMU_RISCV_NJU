    ...
    while (1) 
    {
        ...
        if (memcmp(id, "fmt ", 4) == 0) {
            ...
            if (sz < 16 || sz > sizeof(fmtbuf)) 
            {
                ...
                if (!readExact(fd, fmtbuf, toread)) { break; }
                if (sz > toread && !skipBytes(fd, sz - toread)) { break; }
            } 
            else 
            {
                if (!readExact(fd, fmtbuf, sz)) { break; }
            }
            ...
            if (sz & 1) { if (!skipBytes(fd, 1)) break; }
        ...
        else if (memcmp(id, "data", 4) == 0) 
        {
            ...
            if (!data_ptr) break;
            if (data_size && !readExact(fd, data_ptr, data_size)) { free(data_ptr); data_ptr = NULL; break; }
            have_data = 1;
