...
# 2 "src/monitor/sdb/watchpoint.c" 2
...
# 3 "src/monitor/sdb/watchpoint.c" 2
...
static WP wp_pool[(16)] = {};
static WP *head = 
# 16 "src/monitor/sdb/watchpoint.c" 3 4
                 __null
...
void init_wp_pool()
{
 for (int i = 0; i < (16); ++i)
   {
  wp_pool[i].NO = i;
  wp_pool[i].next = (i == (16) - 1 ? 
# 23 "src/monitor/sdb/watchpoint.c" 3 4
                                     __null 
# 23 "src/monitor/sdb/watchpoint.c"
                                          : &wp_pool[i + 1]);
  memset(wp_pool[i].exprStr, '\0', (32));
  wp_pool[i].lastVal = (word_t) -1;
 }

 head = 
# 28 "src/monitor/sdb/watchpoint.c" 3 4
       __null
...
# 44 "src/monitor/sdb/watchpoint.c" 3 4
(static_cast <bool> (
# 44 "src/monitor/sdb/watchpoint.c"
wp
# 44 "src/monitor/sdb/watchpoint.c" 3 4
...
"wp"
# 44 "src/monitor/sdb/watchpoint.c" 3 4
, "src/monitor/sdb/watchpoint.c", 44, __extension__ __PRETTY_FUNCTION__))
# 44 "src/monitor/sdb/watchpoint.c"
          ;

 wp->next = free_;
 free_ = wp;
 memset(wp->exprStr, '\0', (32));
 wp->lastVal = (word_t) -1;
}
...
void addWp(char* exprs)
{
 if (!free_)
 {
  printf("\33[1;31m" "%s" "\33[0m", "No free watch point.\n");
  return;
 }

 char temp[(32)];

 if (strlen(exprs) > (32) - 1)
 {
  printf("\33[1;31m" "Expression longer that %d.\n" "\33[0m", (32) - 1);
  return;
 }
 ...
 if (!ok)
 {
  printf("\33[1;31m" "%s" "\33[0m", "Expr eval failed.\n");
  return;
 }
...
void delWp(const int n)
{
 if (n < 0 || n >= (16))
 {
  printf("\33[1;31m" "The n should in range %d - %d.\n" "\33[0m", 0, (16) - 1);
  return;
 }

 if (!head)
 {
  printf("\33[1;31m" "%s" "\33[0m", "WP list empty.\n");
  return;
 }
 ...
 if (!head->next)
 {
  ...
  free_wp(toFree);
  head = 
# 108 "src/monitor/sdb/watchpoint.c" 3 4
        __null
# 108 "src/monitor/sdb/watchpoint.c"
            ;
  return;
 }
 ...
 printf("\33[1;31m" "Cannot found watch point %d or no this watch point.\n" "\33[0m", n);
}
...
bool checkEachWpAndPrint()
{
 ...
 for (WP* cur = head; cur; cur = cur->next)
 {
  ...
  if (!success)
  {
   printf("\33[1;31m" "Calculate watch point %d's expression failed.\n" "\33[0m", cur->NO);
   return false;
  }
  ...
  {
   ret = true;
   printf("\33[1;31m" "Watch point [%d] HIT.    Expr: %s.    Old: " "0x%08x" " " "%" 
# 156 "src/monitor/sdb/watchpoint.c" 3 4
         "u" 
# 156 "src/monitor/sdb/watchpoint.c"
         "    New: " "0x%08x" " " "%" 
# 156 "src/monitor/sdb/watchpoint.c" 3 4
         "u" 
# 156 "src/monitor/sdb/watchpoint.c"
         " \n" "\33[0m"
# 167 "src/monitor/sdb/watchpoint.c"
    ,
    cur->NO,
    cur->exprStr,
    cur->lastVal,
...
void printWpByInfoCommand()
{
 if (!head)
 {
  printf("\33[1;31m" "%s" "\33[0m", "Watch point list is empty.\n");
  return;
 }

 printf("\33[1;36m" "%-5s%-32s%-32s\n" "\33[0m", "NO", "Expression", "Last Value");
 for (WP* cur = head; cur; cur = cur->next)
 {
  printf(
   "\33[1;35m" "%-5" 
# 195 "src/monitor/sdb/watchpoint.c" 3 4
  "u" 
# 195 "src/monitor/sdb/watchpoint.c"
  "%-32s%-32" 
# 195 "src/monitor/sdb/watchpoint.c" 3 4
  "u" 
# 195 "src/monitor/sdb/watchpoint.c"
