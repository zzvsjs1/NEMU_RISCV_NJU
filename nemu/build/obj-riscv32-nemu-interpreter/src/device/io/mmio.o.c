...
# 15 "src/device/io/mmio.c"
  ...
  do { printf("\33[1;34m" "[%s:%d %s] " "Add mmio map '%s' at [" "0x%08x" ", " "0x%08x" "]" "\33[0m" "\n", "src/device/io/mmio.c", 18, __func__, maps[nr_map].name, maps[nr_map].low, maps[nr_map].high); do { extern FILE* log_fp; extern bool log_enable(); if (log_fp && log_enable()) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "Add mmio map '%s' at [" "0x%08x" ", " "0x%08x" "]" "\33[0m" "\n", "src/device/io/mmio.c", 18, __func__, maps[nr_map].name, maps[nr_map].low, maps[nr_map].high); fflush(log_fp); } } while (0); } while (0)
                                                             ;
