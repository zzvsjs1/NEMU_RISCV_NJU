...
# 17 "src/utils/log.c" 3 4
       stderr
# 17 "src/utils/log.c"
       , "\33[1;31m" "Can not open '%s'" "\33[0m" "\n", log_file)); extern void assert_fail_msg(); assert_fail_msg(); 
# 17 "src/utils/log.c" 3 4
       (static_cast <bool> (
# 17 "src/utils/log.c"
       fp
...
# 17 "src/utils/log.c"
       "fp"
# 17 "src/utils/log.c" 3 4
       , "src/utils/log.c", 17, __extension__ __PRETTY_FUNCTION__))
# 17 "src/utils/log.c"
       ; } } while (0);
        log_fp = fp;
    }

    do { printf("\33[1;34m" "[%s:%d %s] " "Log is written to %s" "\33[0m" "\n", "src/utils/log.c", 21, __func__, log_file ? log_file : "null"); do { extern FILE* log_fp; extern bool log_enable(); if (log_fp && log_enable()) { fprintf(log_fp, "\33[1;34m" "[%s:%d %s] " "Log is written to %s" "\33[0m" "\n", "src/utils/log.c", 21, __func__, log_file ? log_file : "null"); fflush(log_fp); } } while (0); } while (0);
}

bool log_enable()
{
  return (g_nr_guest_instr >= 0) && (g_nr_guest_instr <= 10000)
                                                       ;
}
