...
enum
{
    ...
    IO_MAP_DIRECT_READ_ALL = IO_MAP_DIRECT_READ_1 | IO_MAP_DIRECT_READ_2 | IO_MAP_DIRECT_READ_4 | IO_MAP_DIRECT_READ_8,
};
...
enum
{
    ...
    IO_MAP_DIRECT_WRITE_ALL = IO_MAP_DIRECT_WRITE_1 | IO_MAP_DIRECT_WRITE_2 | IO_MAP_DIRECT_WRITE_4 | IO_MAP_DIRECT_WRITE_8,
};
...
{
    return map != NULL && (map->direct_read_widths & io_map_direct_read_width(len)) != 0u;
}
...
static inline bool io_direct_write_region_supports(const IODirectWriteRegion *region, int len)
{
    return region != NULL && (region->direct_write_widths & io_map_direct_write_width(len)) != 0u;
}
