...
static void restart()
{
    /* Set the initial instruction pointer and a simple flat 32-bit reset state. */
    cpu.pc = RESET_VECTOR;
    ...
    cpu.es = 0x10;
    cpu.ss = 0x10;
    x86_seg_set_flat(X86_SREG_CS, cpu.cs);
    x86_seg_set_flat(X86_SREG_DS, cpu.ds);
    x86_seg_set_flat(X86_SREG_ES, cpu.es);
    x86_seg_set_flat(X86_SREG_SS, cpu.ss);
    cpu.cr0 = 0x60000011u;
    cpu.cr2 = 0;
    cpu.cr3 = 0;
    cpu.cr4 = 0;
