...
static void mov_sreg_to_rm(Decode *s, int sreg, int rd, word_t addr, int seg)
{
     ...
     */
    if (rd != -1)
        reg_write(rd, X86_WIDTH_DWORD, selector);
    else
        seg_write(seg, addr, X86_WIDTH_WORD, selector);
}
...
static uint16_t addr16_base(int rm, int mod, int *rm_seg)
{
    ...
    case 0:
        *rm_seg = X86_SREG_DS;
        return reg_w(R_BX) + reg_w(R_SI);
    case 1:
        *rm_seg = X86_SREG_DS;
        return reg_w(R_BX) + reg_w(R_DI);
    case 2:
        *rm_seg = X86_SREG_SS;
        return reg_w(R_BP) + reg_w(R_SI);
    case 3:
        *rm_seg = X86_SREG_SS;
        return reg_w(R_BP) + reg_w(R_DI);
    case 4:
        *rm_seg = X86_SREG_DS;
        return reg_w(R_SI);
    case 5:
...
static void decode_rm(Decode *s, int *rm_reg, word_t *rm_addr, int *rm_seg, bool *addr_uses_esp, int *reg, int width, bool addr_size_16)
{
    ...
    {
        load_addr(s, &m, rm_addr, rm_seg, addr_uses_esp, addr_size_16);
        *rm_reg = -1;
    }
}
...
{
    if (rm_reg != -1)
        Rw(rm_reg, width, data);
    else
        Mw(rm_addr, rm_seg, width, data);
}
...
#define simm(width) \
    do \
    { \
        *imm = sext_width(x86_inst_fetch(s, width), width); \
    } while (0)
                           ...
                           word_t *imm, int w, int type, bool addr_size_16)
{
    ...
    {
    case TYPE_r:
        destr(opcode & X86_OPCODE_REG_MASK);
        break;
    case TYPE_I:
        imm();
        break;
    ...
    case TYPE_GP3:
        ...
        break;
    case TYPE_Eb2G:
        decode_rm(s, rs, addr, seg, addr_uses_esp, rd_, X86_WIDTH_BYTE, addr_size_16);
        break;
    case TYPE_Ew2G:
        decode_rm(s, rs, addr, seg, addr_uses_esp, rd_, X86_WIDTH_WORD, addr_size_16);
        break;
    case TYPE_O2a:
        destr(R_EAX);
        *seg = X86_SREG_DS;
        *addr = x86_inst_fetch(s, addr_size_16 ? X86_WIDTH_WORD : X86_WIDTH_DWORD);
        break;
    case TYPE_a2O:
        *rs = R_EAX;
        *seg = X86_SREG_DS;
...
{
    if (cc_eval(cc))
        s->dnpc = branch_target(s->snpc, offset, width);
}
...
static void gp3(Decode *s, int gp_idx, int rd, word_t addr, int seg, int w, word_t imm)
{
    ...
    case 2: // /2 NOT r/m: bitwise complement with no defined flag updates.
        ...
        break;
    case 3:
    { // /3 NEG r/m: subtract operand from zero.
        word_t old_eflags = cpu.eflags;
    ...
    case 4:
    { // /4 MUL: unsigned multiply with implicit accumulator operands.
        if (w == X86_WIDTH_BYTE)
        {
    ...
    case 5:
    { // /5 IMUL: signed multiply with implicit accumulator operands.
        if (w == X86_WIDTH_BYTE)
        {
    ...
    case 6:
    { // /6 DIV: unsigned divide, quotient must fit AL/AX/EAX.
        if (w == X86_WIDTH_BYTE)
        {
    ...
    case 7:
    { // /7 IDIV: signed divide, including the signed-minimum / -1 overflow guard.
        if (w == X86_WIDTH_BYTE)
        {
