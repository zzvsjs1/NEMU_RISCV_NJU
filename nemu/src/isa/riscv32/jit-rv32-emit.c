...
void rv32_jit_reg_cache_restore(rv32_jit_reg_cache_t *regs,
                                    ...
                                    uint32_t len, rv32_jit_tlb_load_patch_t *patch)
{
    ...
    if (!emit_mov_ecx_eax(w) ||
        ...
        !emit_shl_r8_imm(w, 5) ||
        !emit_movabs_rdx(w, (uint64_t)(uintptr_t)rv32_jit_tlb) ||
        !emit_add_r8_rdx(w))
    {
        return false;
    }
...
{
    return emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv32_jit_loop_extra) &&
...
bool rv32_jit_emit_load_store(rv32_jit_writer_t *w,
                                       ...
                                       const uint8_t *target_native)
{
     ...
     */
    if (!emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv32_jit_loop_extra) ||
        !emit_mov_eax_m32_rdx(w) ||
        !emit_add_eax_imm(w, exit_count) ||
        !emit_mov_ecx_eax(w) ||
        !emit_add_ecx_imm(w, exit_count) ||
        !emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv32_jit_entry_budget) ||
        !emit_cmp_ecx_m32_rdx(w) ||
        !emit_jcc_rel32_placeholder(w, 0x87, &over_budget_disp) ||
        !emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv32_jit_loop_extra) ||
...
bool rv32_jit_emit_branch(rv32_jit_writer_t *w, rv32_jit_reg_cache_t *regs,
...
bool rv32_jit_emit_control_flow(rv32_jit_writer_t *w,
 ...
 */
bool rv32_jit_emit_alu(rv32_jit_writer_t *w, rv32_jit_reg_cache_t *regs,
                           uint32_t instr, vaddr_t cur_pc)
{
    const uint32_t opcode = instr & RISCV_OPCODE_MASK;
    const uint32_t rd = bits(instr, 11, 7);
...
static bool jit_fp_opcode_is_memory(uint32_t opcode) { return opcode == RISCV_FP_OPCODE_LOAD || opcode == RISCV_FP_OPCODE_STORE; }
 ...
 * source bytes.
 */
bool rv32_jit_emit_fpu(rv32_jit_writer_t *w, rv32_jit_reg_cache_t *regs, uint32_t instr, vaddr_t pc, uint32_t completed_count,
                           bool *ends_block)
{
    ...
    *ends_block = false;
    ...
    if (!jit_reg_flush_all_dirty(w, regs) || !emit_mov_edi_imm(w, instr) || !emit_mov_esi_imm(w, pc) ||
        !emit_call_abs(w, (uintptr_t)jit_exec_fpu))
    {
        return false;
    }
...
{
    if (!pc_already_set &&
        (!jit_reg_flush_all_dirty(writer, regs) ||
         !emit_set_pc_imm(writer, next_pc)))
    {
        return false;
    }

    return return_loop_count
               ? emit_epilogue_return_loop_count(writer, completed_count)
               : emit_epilogue_return_count(writer, completed_count);
}
