...
#define INSTPAT_MATCH(s, name, type, ... /* execute body */) \
     ...
     */
    INSTPAT("??????? ????? ????? ??? ????? 00001 11", fp_load, N, (void)riscv_fpu_exec(s));
    INSTPAT("??????? ????? ????? ??? ????? 01001 11", fp_store, N, (void)riscv_fpu_exec(s));
    INSTPAT("??????? ????? ????? ??? ????? 10000 11", fp_fmadd, N, (void)riscv_fpu_exec(s));
    INSTPAT("??????? ????? ????? ??? ????? 10001 11", fp_fmsub, N, (void)riscv_fpu_exec(s));
    INSTPAT("??????? ????? ????? ??? ????? 10010 11", fp_fnmsub, N, (void)riscv_fpu_exec(s));
    INSTPAT("??????? ????? ????? ??? ????? 10011 11", fp_fnmadd, N, (void)riscv_fpu_exec(s));
    INSTPAT("??????? ????? ????? ??? ????? 10100 11", fp_op, N, (void)riscv_fpu_exec(s));
#endif
