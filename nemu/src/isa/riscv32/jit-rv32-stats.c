...
void rv32_jit_dump_stats_report(void)
{
    ...
    const uint64_t cache_hit_pct =
        jit_percent_x100(rv32_jit_stats.cache_hits, cache_total);
    const uint64_t avg_compile_len =
        jit_ratio_x100(rv32_jit_stats.compiled_insns, rv32_jit_stats.blocks_compiled);
    const uint64_t avg_exec_len =
        jit_ratio_x100(rv32_jit_stats.executed_insns, rv32_jit_stats.blocks_executed);
    const uint64_t load_direct_pct =
        jit_percent_x100(rv32_jit_stats.helper_load_direct, rv32_jit_stats.helper_loads);
    const uint64_t store_direct_pct =
        jit_percent_x100(rv32_jit_stats.helper_store_direct, rv32_jit_stats.helper_stores);

    Log("jit: exec requests = %" PRIu64
        ", cache hits = %" PRIu64
        ", misses = %" PRIu64
        ", hit rate = %" PRIu64 ".%02" PRIu64 "%%",
        rv32_jit_stats.exec_requests,
        rv32_jit_stats.cache_hits,
        rv32_jit_stats.cache_misses,
        cache_hit_pct / 100u,
        cache_hit_pct % 100u);
    Log("jit: compiled blocks = %" PRIu64
        ...
        rv32_jit_stats.blocks_compiled,
        rv32_jit_stats.blocks_unsupported,
        rv32_jit_stats.fp_blocks_unsupported,
        rv32_jit_stats.native_prefixes_before_fp,
    ...
    Log("jit: executed blocks = %" PRIu64
        ...
        rv32_jit_stats.blocks_executed,
        rv32_jit_stats.executed_insns,
        avg_exec_len / 100u,
        avg_exec_len % 100u,
        rv32_jit_stats.unsupported_hits);
    Log("jit: helper loads = %" PRIu64
        " (%" PRIu64 ".%02" PRIu64 "%% direct PMEM), stores = %" PRIu64
        " (%" PRIu64 ".%02" PRIu64 "%% direct PMEM), complex ops = %" PRIu64,
        rv32_jit_stats.helper_loads,
        load_direct_pct / 100u,
        load_direct_pct % 100u,
        rv32_jit_stats.helper_stores,
        store_direct_pct / 100u,
        store_direct_pct % 100u,
        rv32_jit_stats.helper_complex_ops);
    ...
    Log("jit: invalidation requests = %" PRIu64
        ...
        ", arena resets = %" PRIu64,
        rv32_jit_stats.invalidation_requests,
        rv32_jit_stats.invalidation_page_skips,
        rv32_jit_stats.invalidated_blocks,
        rv32_jit_stats.arena_resets);
#endif
}
