...
static bool rv64_jit_should_test_fp_mmio_boundary(void)
{
    ...
    {
        const char *value = getenv("NEMU_RV64_JIT_TEST_FP_MMIO_BOUNDARY");
        rv64_jit_test_fp_mmio_enabled = value != NULL && value[0] != '\0' && !(value[0] == '0' && value[1] == '\0');
        rv64_jit_test_fp_mmio_initialised = true;
    }

    return rv64_jit_test_fp_mmio_enabled && !rv64_jit_test_fp_mmio_injected;
}
...
static uint32_t rv64_jit_exec_fpu(uint32_t instr, vaddr_t pc)
{
    ...
    const bool inject_test_interrupt = is_memory && rv64_jit_should_test_fp_mmio_boundary();
    ...
    if (is_memory && completed != 0)
    {
        ...
        if (nemu_state.state != NEMU_RUNNING || (!interrupt_was_pending && cpu.INTR))
        {
            /*
...
{
    return emit_u8(w, jit_modrm(HOST_MODRM_MOD_MEM, (uint8_t)reg, HOST_RM_SIB)) && emit_u8(w, HOST_SIB_R10_BASE_RDX_INDEX);
}
...
{
    return emit_u8(w, jit_modrm(HOST_MODRM_MOD_MEM, (uint8_t)reg, HOST_RM_SIB)) && emit_u8(w, HOST_SIB_RCX_BASE_RDX_INDEX);
}
...
{
    return emit_u8(w, jit_modrm(HOST_MODRM_MOD_MEM, (uint8_t)reg, HOST_RM_SIB)) && emit_u8(w, HOST_SIB_RDI_BASE_RDX_INDEX);
}
...
static bool emit_push_saved_hregs(rv64_jit_writer_t *w)
{
     ...
     */
    return emit_u8(w, 0x53) && emit_u8(w, 0x55) && emit_u8(w, 0x41) && emit_u8(w, 0x54) && emit_u8(w, 0x41) && emit_u8(w, 0x55) && emit_u8(w, 0x41) &&
           emit_u8(w, 0x56) && emit_u8(w, 0x41) && emit_u8(w, 0x57) &&
           /* sub rsp, 8 */
           emit_u8(w, 0x48) && emit_u8(w, 0x83) && emit_u8(w, 0xec) && emit_u8(w, 0x08);
}
...
static bool emit_pop_saved_hregs(rv64_jit_writer_t *w)
{
    return
        /* add rsp, 8 */
        emit_u8(w, 0x48) && emit_u8(w, 0x83) && emit_u8(w, 0xc4) && emit_u8(w, 0x08) && emit_u8(w, 0x41) && emit_u8(w, 0x5f) && emit_u8(w, 0x41) &&
        emit_u8(w, 0x5e) && emit_u8(w, 0x41) && emit_u8(w, 0x5d) && emit_u8(w, 0x41) && emit_u8(w, 0x5c) && emit_u8(w, 0x5d) && emit_u8(w, 0x5b);
}
...
{
    return emit_u8(w, 0x49) && emit_u8(w, 0xbb) && emit_u64(w, (uint64_t)(uintptr_t)&cpu);
}
...
{
    return emit_load_cpu_base(w) && emit_movabs_r10(w, (uint64_t)(uintptr_t)guest_to_host(CONFIG_MBASE));
}
...
bool rv64_jit_emit_prologue(rv64_jit_writer_t *w)
{
     ...
     */
    return emit_push_saved_hregs(w) && emit_load_jit_bases(w);
}
...
static bool emit_movabs_reg(rv64_jit_writer_t *w, host_reg_t reg, uint64_t value)
{
    const uint8_t rex = HOST_REX_BASE | HOST_REX_W | (((uint8_t)reg & HOST_REG_EXT_BIT) != 0 ? HOST_REX_B : 0);

    return emit_u8(w, rex) && emit_u8(w, (uint8_t)(0xb8u + ((uint8_t)reg & 7u))) && emit_u64(w, value);
}
 ...
 */
static bool emit_movabs_indirect_pic_address(rv64_jit_writer_t *w, host_reg_t reg)
{
    rv64_jit_indirect_pic_builder_t *builder = w->indirect_pic;

    Assert(builder != NULL && builder->state.used, "jit: missing RV64 indirect PIC builder");
    Assert(builder->state.fixup_count < RV64_JIT_INDIRECT_PIC_MAX_FIXUPS, "jit: too many RV64 indirect PIC address fixups");

    const uint8_t rex = HOST_REX_BASE | HOST_REX_W | (((uint8_t)reg & HOST_REG_EXT_BIT) != 0 ? HOST_REX_B : 0);

    if (!emit_u8(w, rex) || !emit_u8(w, (uint8_t)(0xb8u + ((uint8_t)reg & 7u))))
    {
        return false;
    }
...
static bool emit_movabs_indirect_jump_cache_address(rv64_jit_writer_t *w, host_reg_t reg)
{
    rv64_jit_indirect_jump_cache_builder_t *builder = w->indirect_jump_cache;

    Assert(builder != NULL && builder->state.used, "jit: missing RV64 indirect jump cache builder");
    Assert(builder->state.fixup_count < RV64_JIT_INDIRECT_JUMP_CACHE_MAX_FIXUPS, "jit: too many RV64 indirect jump cache address fixups");
...
static bool emit_movabs_rsi(rv64_jit_writer_t *w, uint64_t value)
{
    return emit_movabs_reg(w, HOST_REG_RSI, value);
}
#endif
...
{
    return emit_u8(w, 0x41) && emit_u8(w, 0x81) && emit_u8(w, 0xc1) && emit_u32(w, value);
}
...
{
    return emit_u8(w, 0x48) && emit_u8(w, 0x83) && emit_u8(w, 0xf9) && emit_u8(w, 0xff);
}
...
{
    return emit_u8(w, 0x48) && emit_u8(w, 0xc1) && emit_u8(w, 0xff) && emit_u8(w, 63);
}
...
static bool emit_imul_rdx_imm32(rv64_jit_writer_t *w, uint32_t value)
{
    ...
    return emit_u8(w, 0x48) && emit_u8(w, 0x69) && emit_u8(w, 0xd2) && emit_u32(w, value);
}
...
static bool emit_cmp_r8b_field_imm8(rv64_jit_writer_t *w, uint32_t offset, uint8_t value)
{
...
static bool emit_cmp_r8q_field_rdx(rv64_jit_writer_t *w, uint32_t offset)
{
    ...
    return emit_u8(w, 0x49) && emit_u8(w, 0x39) && emit_u8(w, 0x50) && emit_u8(w, (uint8_t)offset);
}
...
static bool emit_cmp_r8q_field_r9(rv64_jit_writer_t *w, uint32_t offset)
{
    ...
    return emit_u8(w, 0x4d) && emit_u8(w, 0x39) && emit_u8(w, 0x48) && emit_u8(w, (uint8_t)offset);
}
...
{
    Assert(offset <= INT8_MAX, "jit: RV64 block qword field offset is too large");
    return emit_u8(w, 0x49) && emit_u8(w, 0x39) && emit_u8(w, 0x40) && emit_u8(w, (uint8_t)offset);
}
...
static bool emit_mov_r8_rdxq_field(rv64_jit_writer_t *w, uint32_t offset)
{
    ...
    return emit_u8(w, 0x4c) && emit_u8(w, 0x8b) && emit_u8(w, 0x42) && emit_u8(w, (uint8_t)offset);
}
...
{
    Assert(offset <= INT8_MAX, "jit: RV64 PIC field offset is too large");
    return emit_u8(w, 0x4c) && emit_u8(w, 0x39) && emit_u8(w, 0x4a) && emit_u8(w, (uint8_t)offset);
}
...
{
    Assert(offset <= INT8_MAX, "jit: RV64 indirect jump cache field offset is too large");
    return emit_u8(w, 0x4c) && emit_u8(w, 0x8b) && emit_u8(w, 0x47) && emit_u8(w, (uint8_t)offset);
}
...
{
    Assert(offset <= INT8_MAX, "jit: RV64 indirect jump cache field offset is too large");
    return emit_u8(w, 0x48) && emit_u8(w, 0x8b) && emit_u8(w, 0x47) && emit_u8(w, (uint8_t)offset);
}
...
{
    Assert(offset <= INT8_MAX, "jit: RV64 indirect jump cache field offset is too large");
    return emit_u8(w, 0x4c) && emit_u8(w, 0x89) && emit_u8(w, 0x47) && emit_u8(w, (uint8_t)offset);
}
...
{
    Assert(offset <= INT8_MAX, "jit: RV64 indirect jump cache field offset is too large");
    return emit_u8(w, 0x48) && emit_u8(w, 0x89) && emit_u8(w, 0x47) && emit_u8(w, (uint8_t)offset);
}
...
static bool emit_cmp_r8d_field_imm32(rv64_jit_writer_t *w, uint32_t offset, uint32_t value)
{
...
static bool emit_cmp_r8q_field_imm8(rv64_jit_writer_t *w, uint32_t offset, uint8_t value)
{
    Assert(offset <= INT8_MAX, "jit: RV64 block qword field offset is too large");
    return emit_u8(w, 0x49) && emit_u8(w, 0x83) && emit_u8(w, 0x78) && emit_u8(w, (uint8_t)offset) && emit_u8(w, value);
}
...
{
    Assert(offset <= INT8_MAX, "jit: RV64 block qword field offset is too large");
    return emit_u8(w, 0x49) && emit_u8(w, 0x8b) && emit_u8(w, 0x40) && emit_u8(w, (uint8_t)offset);
}
...
static bool emit_add_ecx_r8d_field(rv64_jit_writer_t *w, uint32_t offset)
{
    ...
    return emit_u8(w, 0x41) && emit_u8(w, 0x03) && emit_u8(w, 0x48) && emit_u8(w, (uint8_t)offset);
}

/* Compare a byte field in the RDX-pointed direct-link block with an immediate. */
static bool emit_cmp_rdxb_field_imm8(rv64_jit_writer_t *w, uint32_t offset, uint8_t value)
{
    Assert(offset <= INT8_MAX, "jit: RV64 block byte field offset is too large");
    return emit_u8(w, 0x80) && emit_u8(w, 0x7a) && emit_u8(w, (uint8_t)offset) && emit_u8(w, value);
}

/* Compare a dword field in the RDX-pointed direct-link block with an immediate. */
static bool emit_cmp_rdxd_field_imm32(rv64_jit_writer_t *w, uint32_t offset, uint32_t value)
{
    Assert(offset <= INT8_MAX, "jit: RV64 block dword field offset is too large");
    return emit_u8(w, 0x81) && emit_u8(w, 0x7a) && emit_u8(w, (uint8_t)offset) && emit_u32(w, value);
}
...
{
    Assert(offset <= INT8_MAX, "jit: RV64 block qword field offset is too large");
    return emit_u8(w, 0x48) && emit_u8(w, 0x39) && emit_u8(w, 0x42) && emit_u8(w, (uint8_t)offset);
}

/* Compare a qword field in the RDX-pointed direct-link block with a sign imm8. */
static bool emit_cmp_rdxq_field_imm8(rv64_jit_writer_t *w, uint32_t offset, uint8_t value)
{
    Assert(offset <= INT8_MAX, "jit: RV64 block qword field offset is too large");
    return emit_u8(w, 0x48) && emit_u8(w, 0x83) && emit_u8(w, 0x7a) && emit_u8(w, (uint8_t)offset) && emit_u8(w, value);
}
...
{
    Assert(offset <= INT8_MAX, "jit: RV64 block qword field offset is too large");
    return emit_u8(w, 0x48) && emit_u8(w, 0x8b) && emit_u8(w, 0x42) && emit_u8(w, (uint8_t)offset);
}
...
static bool emit_test_r8d_field_imm32(rv64_jit_writer_t *w, uint32_t offset, uint32_t value)
{
    Assert(offset <= INT8_MAX, "jit: RV64 DTLB dword field offset is too large");
...
{
    Assert(offset <= INT8_MAX, "jit: RV64 DTLB qword field offset is too large");
    return emit_u8(w, 0x49) && emit_u8(w, 0x8b) && emit_u8(w, 0x50) && emit_u8(w, (uint8_t)offset);
}
...
static bool emit_cmp_u16_ref_zero_rax_r8(rv64_jit_writer_t *w)
{
    ...
    return emit_u8(w, 0x66) && emit_u8(w, 0x42) && emit_u8(w, 0x83) && emit_u8(w, 0x3c) && emit_u8(w, 0x40) && emit_u8(w, 0x00);
}
...
static bool emit_cmp_u32_ref_zero_rax_r8(rv64_jit_writer_t *w)
{
    ...
    return emit_u8(w, 0x42) && emit_u8(w, 0x83) && emit_u8(w, 0x3c) && emit_u8(w, 0x80) && emit_u8(w, 0x00);
}
...
static bool emit_mov_edi_imm32(rv64_jit_writer_t *w, uint32_t imm)
{
    return emit_u8(w, 0xbf) && emit_u32(w, imm);
}
#endif
...
{
    return emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv64_jit_loop_extra) && emit_mov_eax_m32_rdx(w) && emit_add_eax_imm32(w, count) &&
           emit_return_eax(w);
}
...
static bool emit_load_gpr_hreg(rv64_jit_writer_t *w, rv64_jit_hreg_t hreg, uint32_t reg)
{
    const uint8_t dst = jit_hreg_x86_reg(hreg);
    const uint8_t base = 11;

    /* `REX.W 8b /r` is `mov r64, qword ptr [r11 + disp32]`. */
    return emit_rex64(w, dst, base) && emit_u8(w, 0x8b) && emit_u8(w, jit_modrm(2, dst, base)) && emit_u32(w, jit_gpr_offset(reg));
}

/* Store one cached 64-bit host register back into `cpu.gpr[reg]`. */
static bool emit_store_gpr_hreg(rv64_jit_writer_t *w, uint32_t reg, rv64_jit_hreg_t hreg)
{
    const uint8_t src = jit_hreg_x86_reg(hreg);
    const uint8_t base = 11;

    /* `REX.W 89 /r` is `mov qword ptr [r11 + disp32], r64`. */
    return emit_rex64(w, src, base) && emit_u8(w, 0x89) && emit_u8(w, jit_modrm(2, src, base)) && emit_u32(w, jit_gpr_offset(reg));
}

#ifdef CONFIG_RISCV_FPU
/* Load one raw 64-bit CPU-state field through the fixed R11 base. */
static bool emit_load_cpu_u64_reg(rv64_jit_writer_t *w, host_reg_t dst, uint32_t offset)
{
    return emit_rex64(w, (uint8_t)dst, HOST_REG_R11) && emit_u8(w, 0x8b) &&
           emit_u8(w, jit_modrm(HOST_MODRM_MOD_DISP32, (uint8_t)dst, HOST_REG_R11)) && emit_u32(w, offset);
}

/* Load a 32-bit CPU-state field, zero-extending it in the selected host reg. */
static bool emit_load_cpu_u32_reg(rv64_jit_writer_t *w, host_reg_t dst, uint32_t offset)
{
    return emit_rex32_if_needed(w, (uint8_t)dst, HOST_REG_R11) && emit_u8(w, 0x8b) &&
           emit_u8(w, jit_modrm(HOST_MODRM_MOD_DISP32, (uint8_t)dst, HOST_REG_R11)) && emit_u32(w, offset);
}

/* Store one raw 64-bit host register into CPU state through R11. */
static bool emit_store_cpu_u64_reg(rv64_jit_writer_t *w, uint32_t offset, host_reg_t src)
{
    return emit_rex64(w, (uint8_t)src, HOST_REG_R11) && emit_u8(w, 0x89) &&
           emit_u8(w, jit_modrm(HOST_MODRM_MOD_DISP32, (uint8_t)src, HOST_REG_R11)) && emit_u32(w, offset);
}

/* Test one 64-bit CPU-state field against a sign-extended imm32 mask. */
static bool emit_test_cpu_u64_imm32(rv64_jit_writer_t *w, uint32_t offset, uint32_t mask)
{
    /*
     ...
     */
    return emit_rex64(w, 0, HOST_REG_R11) && emit_u8(w, 0xf7) && emit_u8(w, jit_modrm(HOST_MODRM_MOD_DISP32, 0, HOST_REG_R11)) &&
...
static bool emit_mov_rax_hreg(rv64_jit_writer_t *w, rv64_jit_hreg_t hreg)
{
    ...
    return emit_rex64(w, src, 0) && emit_u8(w, 0x89) && emit_u8(w, jit_modrm(3, src, 0));
}
...
static bool emit_mov_rcx_hreg(rv64_jit_writer_t *w, rv64_jit_hreg_t hreg)
{
    ...
    return emit_rex64(w, src, 1) && emit_u8(w, 0x89) && emit_u8(w, jit_modrm(3, src, 1));
}
...
static bool emit_mov_hreg_rax(rv64_jit_writer_t *w, rv64_jit_hreg_t hreg)
{
    ...
    return emit_rex64(w, 0, dst) && emit_u8(w, 0x89) && emit_u8(w, jit_modrm(3, 0, dst));
}

/* Copy one cached host register to another. */
static bool emit_mov_hreg_hreg(rv64_jit_writer_t *w, rv64_jit_hreg_t dst, rv64_jit_hreg_t src)
{
    ...
    return emit_rex64(w, src_reg, dst_reg) && emit_u8(w, 0x89) && emit_u8(w, jit_modrm(3, src_reg, dst_reg));
}

/* Emit `hreg op imm32` using x86 group-1 immediate ALU operations. */
static bool emit_hreg_imm32_alu64(rv64_jit_writer_t *w, rv64_jit_hreg_t hreg, uint8_t subop, int32_t imm)
{
    const uint8_t dst = jit_hreg_x86_reg(hreg);

    return emit_rex64(w, subop, dst) && emit_u8(w, 0x81) && emit_u8(w, jit_modrm(3, subop, dst)) && emit_u32(w, (uint32_t)imm);
}

/* Emit `dst op src` directly between two cached 64-bit host registers. */
static bool emit_hreg_hreg_alu64(rv64_jit_writer_t *w, rv64_jit_hreg_t dst, rv64_jit_hreg_t src, uint8_t opcode)
{
    const uint8_t dst_reg = jit_hreg_x86_reg(dst);
    const uint8_t src_reg = jit_hreg_x86_reg(src);

    return emit_rex64(w, src_reg, dst_reg) && emit_u8(w, opcode) && emit_u8(w, jit_modrm(3, src_reg, dst_reg));
}

/* Compare two cached 64-bit values directly without scratch-register moves. */
static bool emit_cmp_hreg_hreg(rv64_jit_writer_t *w, rv64_jit_hreg_t lhs, rv64_jit_hreg_t rhs)
{
    const uint8_t lhs_reg = jit_hreg_x86_reg(lhs);
    const uint8_t rhs_reg = jit_hreg_x86_reg(rhs);

    return emit_rex64(w, rhs_reg, lhs_reg) && emit_u8(w, 0x39) && emit_u8(w, jit_modrm(3, rhs_reg, lhs_reg));
}

/* Set equality flags by testing one cached 64-bit value against itself. */
static bool emit_test_hreg_hreg(rv64_jit_writer_t *w, rv64_jit_hreg_t hreg)
{
    const uint8_t reg = jit_hreg_x86_reg(hreg);

    return emit_rex64(w, reg, reg) && emit_u8(w, 0x85) && emit_u8(w, jit_modrm(3, reg, reg));
}

/* Materialise one condition-code result directly in a cached host register. */
static bool emit_setcc_hreg(rv64_jit_writer_t *w, rv64_jit_hreg_t hreg, uint8_t setcc_opcode)
{
    const uint8_t reg = jit_hreg_x86_reg(hreg);
    const bool needs_byte_rex = (reg & 7u) >= 4u || (reg & HOST_REG_EXT_BIT) != 0;
    ...
    return (!needs_byte_rex || emit_u8(w, setcc_rex)) && emit_u8(w, 0x0f) && emit_u8(w, setcc_opcode) && emit_u8(w, jit_modrm(3, 0, reg)) &&
           (!needs_byte_rex || emit_u8(w, movzx_rex)) && emit_u8(w, 0x0f) && emit_u8(w, 0xb6) && emit_u8(w, jit_modrm(3, reg, reg));
}

/* Emit `dst32 op src32` directly between cached host registers. */
static bool emit_hreg_hreg_alu32(rv64_jit_writer_t *w, rv64_jit_hreg_t dst, rv64_jit_hreg_t src, uint8_t opcode)
{
    const uint8_t dst_reg = jit_hreg_x86_reg(dst);
    const uint8_t src_reg = jit_hreg_x86_reg(src);
...
static bool emit_hreg_hreg_imul32(rv64_jit_writer_t *w, rv64_jit_hreg_t dst, rv64_jit_hreg_t src)
{
    const uint8_t dst_reg = jit_hreg_x86_reg(dst);
    const uint8_t src_reg = jit_hreg_x86_reg(src);

    return emit_rex32_if_needed(w, dst_reg, src_reg) && emit_u8(w, 0x0f) && emit_u8(w, 0xaf) && emit_u8(w, jit_modrm(3, dst_reg, src_reg));
}
...
static bool emit_hreg_sext32(rv64_jit_writer_t *w, rv64_jit_hreg_t hreg)
{
    ...
    return emit_rex64(w, reg, reg) && emit_u8(w, 0x63) && emit_u8(w, jit_modrm(3, reg, reg));
}

/* Emit one immediate shift directly into a cached 64-bit host register. */
static bool emit_shift_hreg_imm(rv64_jit_writer_t *w, rv64_jit_hreg_t hreg, uint8_t subop, uint8_t shamt)
{
    const uint8_t dst = jit_hreg_x86_reg(hreg);

    return emit_rex64(w, subop, dst) && emit_u8(w, 0xc1) && emit_u8(w, jit_modrm(3, subop, dst)) && emit_u8(w, shamt);
}

/* Load a full-width constant into one cached host register. */
static bool emit_mov_hreg_imm64(rv64_jit_writer_t *w, rv64_jit_hreg_t hreg, uint64_t value)
{
    const uint8_t dst = jit_hreg_x86_reg(hreg);
    ...
    if ((int64_t)value >= INT32_MIN && (int64_t)value <= INT32_MAX)
    {
         ...
         */
        return emit_rex64(w, 0, dst) && emit_u8(w, 0xc7) && emit_u8(w, jit_modrm(3, 0, dst)) && emit_u32(w, (uint32_t)value);
    }

    /* `REX.W b8+rd imm64` is the full movabs form for arbitrary RV64 values. */
    return emit_rex64(w, 0, dst) && emit_u8(w, (uint8_t)(0xb8u + (dst & 7u))) && emit_u64(w, value);
}
...
void rv64_jit_reg_cache_restore(rv64_jit_reg_cache_t *regs, const rv64_jit_reg_cache_t *snapshot)
{
    *regs = *snapshot;
}

/* Compare cache metadata field by field so structure padding is irrelevant. */
static bool rv64_jit_reg_cache_matches(const rv64_jit_reg_cache_t *regs, const rv64_jit_reg_cache_t *expected)
{
    if (regs->slot_count != expected->slot_count || regs->next_age != expected->next_age || regs->current_use_mask != expected->current_use_mask ||
    ...
    for (uint32_t i = 0; i < RV64_JIT_HREG_COUNT; i++)
    {
        ...
        if (actual_slot->valid != expected_slot->valid || actual_slot->loaded != expected_slot->loaded ||
            actual_slot->dirty != expected_slot->dirty || actual_slot->guest_reg != expected_slot->guest_reg ||
            actual_slot->age != expected_slot->age || actual_slot->hreg != expected_slot->hreg)
        {
            return false;
        }
    }
 ...
 */
void rv64_jit_emitter_checkpoint_capture(rv64_jit_emitter_checkpoint_t *checkpoint, const rv64_jit_writer_t *w, const rv64_jit_reg_cache_t *regs,
                                         const rv64_jit_mmio_route_builder_t *mmio_routes)
{
    ...
    Assert(regs != NULL, "jit: missing RV64 register cache for checkpoint capture");
    Assert(w->links != NULL, "jit: missing RV64 link builder for checkpoint capture");
    Assert(w->indirect_pic != NULL, "jit: missing RV64 indirect PIC builder for checkpoint capture");
    Assert(w->indirect_jump_cache != NULL, "jit: missing RV64 jump-cache builder for checkpoint capture");
    Assert(mmio_routes != NULL, "jit: missing RV64 MMIO builder for checkpoint capture");
 ...
 */
void rv64_jit_emitter_checkpoint_restore(const rv64_jit_emitter_checkpoint_t *checkpoint, rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs,
                                         rv64_jit_mmio_route_builder_t *mmio_routes)
{
    Assert(checkpoint != NULL, "jit: missing RV64 emitter checkpoint");
    Assert(w != NULL, "jit: missing RV64 writer for checkpoint restore");
    Assert(regs != NULL, "jit: missing RV64 register cache for checkpoint restore");
    Assert(w->links != NULL, "jit: missing RV64 link builder for checkpoint restore");
    Assert(w->indirect_pic != NULL, "jit: missing RV64 indirect PIC builder for checkpoint restore");
    Assert(w->indirect_jump_cache != NULL, "jit: missing RV64 jump-cache builder for checkpoint restore");
    Assert(mmio_routes != NULL, "jit: missing RV64 MMIO builder for checkpoint restore");
     ...
     */
    Assert(w->cur == checkpoint->writer_cur, "jit: RV64 emitter checkpoint did not restore the writer cursor");
    Assert(rv64_jit_reg_cache_matches(regs, &checkpoint->regs), "jit: RV64 emitter checkpoint did not restore register metadata");
    Assert(w->links->state.count == checkpoint->links.count, "jit: RV64 emitter checkpoint did not restore link progress");
    Assert(w->indirect_pic->state.fixup_count == checkpoint->indirect_pic.fixup_count &&
               w->indirect_pic->state.kind == checkpoint->indirect_pic.kind && w->indirect_pic->state.used == checkpoint->indirect_pic.used,
           "jit: RV64 emitter checkpoint did not restore indirect PIC progress");
    Assert(w->indirect_jump_cache->state.fixup_count == checkpoint->indirect_jump_cache.fixup_count &&
               w->indirect_jump_cache->state.used == checkpoint->indirect_jump_cache.used,
           "jit: RV64 emitter checkpoint did not restore jump-cache progress");
    Assert(mmio_routes->state.site_count == checkpoint->mmio_routes.site_count &&
               mmio_routes->state.fixup_count == checkpoint->mmio_routes.fixup_count,
...
void rv64_jit_emitter_checkpoint_validate_once(void)
{
    ...
    const rv64_jit_emitted_site_stats_t saved_emitted_sites = rv64_jit_stats.emitted_sites;
    memset(&rv64_jit_stats.emitted_sites, 0xa5, sizeof(rv64_jit_stats.emitted_sites));

    rv64_jit_emitter_checkpoint_t checkpoint;
    rv64_jit_emitter_checkpoint_capture(&checkpoint, &w, &regs, &mmio_routes);

    /* Validate capture independently before the live state is disturbed. */
    Assert(checkpoint.writer_cur == w.cur, "jit: RV64 emitter checkpoint did not capture the writer cursor");
    Assert(rv64_jit_reg_cache_matches(&checkpoint.regs, &regs), "jit: RV64 emitter checkpoint did not capture register metadata");
    Assert(checkpoint.links.count == links.state.count, "jit: RV64 emitter checkpoint did not capture link progress");
    Assert(checkpoint.indirect_pic.fixup_count == indirect_pic.state.fixup_count && checkpoint.indirect_pic.kind == indirect_pic.state.kind &&
               checkpoint.indirect_pic.used == indirect_pic.state.used,
           "jit: RV64 emitter checkpoint did not capture indirect PIC progress");
    Assert(checkpoint.indirect_jump_cache.fixup_count == indirect_jump_cache.state.fixup_count &&
               checkpoint.indirect_jump_cache.used == indirect_jump_cache.state.used,
           "jit: RV64 emitter checkpoint did not capture jump-cache progress");
    Assert(checkpoint.mmio_routes.site_count == mmio_routes.state.site_count && checkpoint.mmio_routes.fixup_count == mmio_routes.state.fixup_count,
           "jit: RV64 emitter checkpoint did not capture MMIO route progress");
    Assert(memcmp(&checkpoint.emitted_sites, &rv64_jit_stats.emitted_sites, sizeof(checkpoint.emitted_sites)) == 0,
    ...
    indirect_jump_cache.state = (rv64_jit_indirect_jump_cache_builder_state_t){0};
    mmio_routes.state = (rv64_jit_mmio_route_builder_state_t){0};
    memset(&rv64_jit_stats.emitted_sites, 0x5a, sizeof(rv64_jit_stats.emitted_sites));

    rv64_jit_emitter_checkpoint_restore(&checkpoint, &w, &regs, &mmio_routes);
    Assert(w.overflowed, "jit: RV64 emitter checkpoint cleared attempt overflow state");
...
void rv64_jit_reg_cache_set_liveness(rv64_jit_reg_cache_t *regs, uint32_t current_use_mask, uint32_t live_after_mask)
{
...
static rv64_jit_reg_slot_t *jit_reg_find(rv64_jit_reg_cache_t *regs, uint32_t reg)
{
...
static uint32_t jit_reg_loaded_mapping_count(const rv64_jit_reg_cache_t *regs)
{
    uint32_t count = 0;
...
static uint32_t jit_reg_dirty_mapping_count(const rv64_jit_reg_cache_t *regs)
{
    uint32_t count = 0;
 ...
 */
static uint32_t jit_reg_discard_helper_written_guest(rv64_jit_reg_cache_t *regs, uint32_t reg)
{
...
static void jit_reg_apply_fp_helper_effect(rv64_jit_reg_cache_t *regs, riscv_fpu_gpr_effect_t effect, uint32_t input_flushes, uint32_t trap_stores)
{
    ...
    for (uint32_t reg = 1; reg < RISCV_GPR_NUM; reg++)
    {
        ...
        {
            invalidated += jit_reg_discard_helper_written_guest(regs, reg);
        }
    }

    JIT_STAT_INC(emitted_sites.fp_helper_gpr_effect_sites);
    JIT_STAT_ADD(emitted_sites.fp_helper_gpr_mappings_preserved, jit_reg_loaded_mapping_count(regs));
    JIT_STAT_ADD(emitted_sites.fp_helper_gpr_selective_invalidations, invalidated);
    JIT_STAT_ADD(emitted_sites.fp_helper_gpr_input_flushes, input_flushes);
    JIT_STAT_ADD(emitted_sites.fp_helper_gpr_dirty_mappings_preserved, jit_reg_dirty_mapping_count(regs));
 ...
 */
static bool jit_reg_can_defer_fp_helper_sync(riscv_fpu_gpr_effect_t effect)
{
#ifdef CONFIG_DIFFTEST
    (void)effect;
    return false;
#else
    return rv64_jit_fp_gpr_effects_enabled() && effect.precise && effect.trap_preserves_gprs;
#endif
}
#endif
 ...
 */
static bool jit_reg_flush_mask(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t mask, uint32_t *flushed)
{
...
static bool jit_reg_flush_all_dirty(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs)
{
    for (uint32_t i = 0; i < regs->slot_count; i++)
    {
...
static bool jit_reg_emit_flush_all_dirty(rv64_jit_writer_t *w, const rv64_jit_reg_cache_t *regs)
{
    for (uint32_t i = 0; i < regs->slot_count; i++)
    {
...
static rv64_jit_reg_slot_t *jit_reg_choose_slot(rv64_jit_reg_cache_t *regs)
{
    ...
    for (uint32_t i = 0; i < regs->slot_count; i++)
    {
        ...
        if (oldest_unpinned == NULL || slot->age < oldest_unpinned->age)
        {
            oldest_unpinned = slot;
        }

        if ((regs->live_after_mask & guest_bit) == 0 && (oldest_dead == NULL || slot->age < oldest_dead->age))
        {
            oldest_dead = slot;
        }
    }
    ...
    if (oldest_dead != NULL)
    {
        ...
        if (oldest_unpinned != oldest_dead && oldest_unpinned != NULL && (regs->live_after_mask & (1u << oldest_unpinned->guest_reg)) != 0)
        {
...
static rv64_jit_reg_slot_t *jit_reg_loaded_slot(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t reg)
{
 ...
 * lap.
 */
bool rv64_jit_prepare_stable_loop_regs(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t reg_mask, uint32_t loop_insn_count)
{
    if ((reg_mask & 1u) != 0 || (uint32_t)__builtin_popcount(reg_mask) > RV64_JIT_HREG_COUNT || loop_insn_count == 0)
    {
        return false;
    }
    ...
    {
        if ((reg_mask & (1u << reg)) != 0 && jit_reg_loaded_slot(w, regs, reg) == NULL)
        {
            return false;
        }
    }

    return emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv64_jit_loop_extra) && emit_mov_r9d_m32_rdx(w) &&
           emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv64_jit_entry_budget) && emit_mov_esi_m32_rdx(w) && emit_sub_esi_r9d(w) &&
 ...
 */
static bool jit_reg_try_emit_cached_compare(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t rd, uint32_t rs1, uint32_t rs2,
                                            uint8_t setcc_opcode, bool *handled)
{
    *handled = false;
    ...
    if (dst == NULL || lhs == NULL || rhs == NULL || !dst->loaded || !lhs->loaded || !rhs->loaded)
    {
        return true;
    }
    ...
    *handled = true;
    ...
    if (!emit_cmp_hreg_hreg(w, lhs->hreg, rhs->hreg) || !emit_setcc_hreg(w, dst->hreg, setcc_opcode))
    {
        return false;
    }
...
static void jit_reg_mark_written(rv64_jit_reg_cache_t *regs, rv64_jit_reg_slot_t *slot)
{
    slot->loaded = true;
    slot->dirty = true;
    slot->age = regs->next_age++;
...
static bool emit_store_pc_imm(rv64_jit_writer_t *w, vaddr_t pc)
{
    return emit_movabs_rax(w, pc) &&
           ...
           emit_u8(w, 0x49) && emit_u8(w, 0x89) && emit_u8(w, 0x83) && emit_u32(w, jit_pc_offset());
}

/* Store a dynamic guest PC already held in RAX. */
static bool emit_store_rax_pc(rv64_jit_writer_t *w)
{
    return emit_u8(w, 0x49) && emit_u8(w, 0x89) && emit_u8(w, 0x83) && emit_u32(w, jit_pc_offset());
}
...
static bool emit_jcc_rel32_placeholder(rv64_jit_writer_t *w, uint8_t jcc_opcode, uint8_t **disp)
{
...
{
    return emit_movabs_rax(w, (uint64_t)target) && emit_u8(w, 0xff) && emit_u8(w, 0xd0);
}
...
static bool emit_inc_jit_stat_counter_preserve_rax(rv64_jit_writer_t *w, uint64_t *counter)
{
#if RV64_JIT_STATS
...
static uint8_t mmio_route_reserve_site(rv64_jit_mmio_route_builder_t *routes, uint64_t guest_addr, uint64_t host_ptr)
{
    if (routes == NULL || routes->state.site_count >= RV64_JIT_MMIO_ROUTE_MAX_SITES)
    {
    ...
    routes->initial_routes[site] = (rv64_jit_mmio_route_t){
...
static bool mmio_route_record_fixup(rv64_jit_mmio_route_builder_t *routes, uint8_t site, rv64_jit_mmio_route_field_t field, uint8_t *disp32,
                                    const uint8_t *next_ip)
{
    Assert(routes != NULL && site < routes->state.site_count, "jit: invalid direct-MMIO route site %u", site);
    Assert(field == RV64_JIT_MMIO_ROUTE_TAG || field == RV64_JIT_MMIO_ROUTE_HOST, "jit: invalid direct-MMIO route field %u", field);
    Assert(routes->state.fixup_count < RV64_JIT_MMIO_ROUTE_MAX_FIXUPS, "jit: too many direct-MMIO route fixups");

    routes->fixups[routes->state.fixup_count++] = (rv64_jit_mmio_route_fixup_t){
        .disp32 = disp32,
        .next_ip = next_ip,
        .site = site,
        .field = (uint8_t)field,
...
static bool emit_mmio_route_load_host_rdx(rv64_jit_writer_t *w, rv64_jit_mmio_route_builder_t *routes, uint8_t site)
{
    ...
    return emit_u32(w, 0) && mmio_route_record_fixup(routes, site, RV64_JIT_MMIO_ROUTE_HOST, disp32, w->cur);
}

/* Emit `mov rdi, qword ptr [rip + route.host]`. */
static bool emit_mmio_route_load_host_rdi(rv64_jit_writer_t *w, rv64_jit_mmio_route_builder_t *routes, uint8_t site)
{
...
static bool emit_mmio_route_cmp_tag_rax(rv64_jit_writer_t *w, rv64_jit_mmio_route_builder_t *routes, uint8_t site)
{
    ...
    uint8_t *disp32 = w->cur;
    return emit_u32(w, 0) && mmio_route_record_fixup(routes, site, RV64_JIT_MMIO_ROUTE_TAG, disp32, w->cur);
}

/* Emit `mov qword ptr [rip + route.tag], rax`. */
static bool emit_mmio_route_store_tag_rax(rv64_jit_writer_t *w, rv64_jit_mmio_route_builder_t *routes, uint8_t site)
{
...
static bool emit_mmio_route_store_tag_rdi(rv64_jit_writer_t *w, rv64_jit_mmio_route_builder_t *routes, uint8_t site)
{
...
static bool emit_mmio_route_store_host_rdx(rv64_jit_writer_t *w, rv64_jit_mmio_route_builder_t *routes, uint8_t site)
{
...
static bool emit_mmio_route_store_host_rdi(rv64_jit_writer_t *w, rv64_jit_mmio_route_builder_t *routes, uint8_t site)
{
    if (!emit_u8(w, 0x48) || !emit_u8(w, 0x89) || !emit_u8(w, 0x3d))
    {
        return false;
 ...
 */
bool rv64_jit_finalise_mmio_routes(rv64_jit_writer_t *w, rv64_jit_mmio_route_builder_t *routes)
{
    Assert(routes != NULL, "jit: missing direct-MMIO route builder");

    if (routes->state.site_count == 0)
    {
        Assert(routes->state.fixup_count == 0, "jit: route fixups exist without route sites");
        return true;
    }

    Assert(routes->state.site_count <= RV64_JIT_MMIO_ROUTE_MAX_SITES, "jit: too many direct-MMIO route sites");

    const uintptr_t aligned = rv64_jit_align_up((uintptr_t)w->cur, RV64_JIT_MMIO_ROUTE_LINE_SIZE);
    ...
    {
        const rv64_jit_mmio_route_t initial = i < routes->state.site_count ? routes->initial_routes[i] : (rv64_jit_mmio_route_t){0};

        if (!emit_u64(w, initial.guest_addr_tag) || !emit_u64(w, initial.host_ptr))
        {
            return false;
        }
    }
    ...
    for (uint32_t i = 0; i < routes->state.fixup_count; i++)
    {
        ...
        Assert(fixup->site < routes->state.site_count, "jit: route fixup refers to absent site %u", fixup->site);

        const size_t field_offset =
            fixup->field == RV64_JIT_MMIO_ROUTE_HOST ? offsetof(rv64_jit_mmio_route_t, host_ptr) : offsetof(rv64_jit_mmio_route_t, guest_addr_tag);
        const uint8_t *target = route_base + (size_t)fixup->site * sizeof(rv64_jit_mmio_route_t) + field_offset;
        const int64_t rel = target - fixup->next_ip;

        Assert(rel >= INT32_MIN && rel <= INT32_MAX, "jit: direct-MMIO route disp32 is out of range");
 ...
 */
bool rv64_jit_finalise_indirect_jump_cache(rv64_jit_writer_t *w, rv64_jit_indirect_jump_cache_builder_t *builder)
{
    Assert(builder != NULL, "jit: missing RV64 indirect jump cache builder");

    if (!builder->state.used)
    {
        Assert(builder->state.fixup_count == 0, "jit: indirect jump cache fixup exists without a site");
        return true;
    }

    Assert(builder->state.fixup_count == RV64_JIT_INDIRECT_JUMP_CACHE_MAX_FIXUPS, "jit: invalid RV64 indirect jump cache fixup count %u",
           builder->state.fixup_count);

    const uintptr_t aligned = rv64_jit_align_up((uintptr_t)w->cur, RV64_JIT_INDIRECT_JUMP_CACHE_LINE_SIZE);
    const size_t padding = (size_t)(aligned - (uintptr_t)w->cur);
    const size_t available = (size_t)(w->end - w->cur);

    if (padding > available || sizeof(rv64_jit_indirect_jump_cache_t) > available - padding)
    {
        w->overflowed = true;
        return false;
    }
    ...
    rv64_jit_indirect_jump_cache_t *persistent = (rv64_jit_indirect_jump_cache_t *)w->cur;
 ...
 */
bool rv64_jit_finalise_indirect_pic(rv64_jit_writer_t *w, rv64_jit_indirect_pic_builder_t *builder, rv64_jit_indirect_pic_t **pic)
{
    ...
    *pic = NULL;
    ...
    if (!builder->state.used)
    {
        Assert(builder->state.fixup_count == 0, "jit: indirect PIC fixups exist without a site");
        return true;
    }

    Assert(builder->state.kind < RV64_JIT_INDIRECT_PIC_KIND_COUNT, "jit: invalid RV64 indirect PIC builder kind %u", builder->state.kind);
    Assert(builder->state.fixup_count > 0 && builder->state.fixup_count <= RV64_JIT_INDIRECT_PIC_MAX_FIXUPS,
           "jit: invalid RV64 indirect PIC fixup count %u", builder->state.fixup_count);

    const uintptr_t aligned = rv64_jit_align_up((uintptr_t)w->cur, RV64_JIT_INDIRECT_PIC_LINE_SIZE);
    const size_t padding = (size_t)(aligned - (uintptr_t)w->cur);
    const size_t available = (size_t)(w->end - w->cur);

    if (padding > available || sizeof(rv64_jit_indirect_pic_t) > available - padding)
    {
        w->overflowed = true;
        return false;
    }
    ...
    rv64_jit_indirect_pic_t *persistent = (rv64_jit_indirect_pic_t *)w->cur;
    memset(persistent, 0, sizeof(*persistent));
    persistent->kind = builder->state.kind;

    for (uint32_t i = 0; i < RV64_JIT_INDIRECT_PIC_WAYS; i++)
    {
        Assert(builder->selector_disps[i] != NULL && builder->target_disps[i] != NULL && builder->guarded_paths[i] != NULL &&
...
static bool emit_guard_store_not_compiled_source(rv64_jit_writer_t *w, uint32_t len, uint8_t **cross_chunk_disp, uint8_t **source_chunk_disp);
static bool emit_guard_store_not_translation_dependency(rv64_jit_writer_t *w, uint8_t **data_page_table_disp, uint8_t **ifetch_page_table_disp);
static bool emit_alignment_guard_al(rv64_jit_writer_t *w, uint32_t len, uint8_t **slow_disp);
static bool emit_guard_bare_address_in_pmem(rv64_jit_writer_t *w, uint32_t len, uint8_t **slow_disp);
static void patch_tlb_guard(const rv64_jit_tlb_guard_patch_t *patch, const uint8_t *slow_path);
static bool emit_inline_sv39_load_fast_path(rv64_jit_writer_t *w, uint32_t funct3, uint32_t len, rv64_jit_tlb_guard_patch_t *patch);
static bool emit_inline_sv39_store_address(rv64_jit_writer_t *w, uint32_t len, rv64_jit_tlb_guard_patch_t *patch);
...
static bool emit_fp_unbox_s_rax(rv64_jit_writer_t *w)
{
    ...
    if (!emit_mov_rdx_rax(w) || !emit_shr_rdx_imm(w, 32) || !emit_cmp_edx_neg_one(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &boxed_disp) ||
...
{
    const uint64_t dirty_mask = (uint64_t)RISCV_MSTATUS_FS_DIRTY | (uint64_t)RISCV_MSTATUS_SD;

    return emit_load_cpu_u64_reg(w, HOST_REG_RAX, jit_mstatus_offset()) && emit_movabs_rdx(w, dirty_mask) && emit_rax_rdx_alu64(w, HOST_ALU_OR) &&
           emit_store_cpu_u64_reg(w, jit_mstatus_offset(), HOST_REG_RAX);
}

/* Emit one terminal FCLASS result arm and remember its jump to the join. */
static bool emit_fp_class_result_arm(rv64_jit_writer_t *w, uint32_t bit, uint8_t **done_disps, uint32_t *done_count)
{
    Assert(bit < 10u, "jit: invalid FCLASS result bit");
    Assert(*done_count < 10u, "jit: too many FCLASS result arms");

    return emit_mov_eax_imm32(w, 1u << bit) && emit_jmp_rel32_placeholder(w, &done_disps[(*done_count)++]);
}
...
static bool emit_fp_classify_s_rax(rv64_jit_writer_t *w)
{
    ...
    if (!emit_mov_ecx_eax(w) || !emit_and_ecx_imm32(w, RV64_JIT_FP32_EXPONENT_MASK) || !emit_cmp_ecx_imm32(w, RV64_JIT_FP32_EXPONENT_MASK) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &exponent_all_ones_disp) || !emit_test_ecx_ecx(w) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &exponent_zero_disp) || !emit_test_eax_imm32(w, RV64_JIT_FP32_SIGN_MASK) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &positive_normal_disp) || !emit_fp_class_result_arm(w, 1, done_disps, &done_count))
    {
        return false;
    }
    ...
    if (!emit_mov_ecx_eax(w) || !emit_and_ecx_imm32(w, RV64_JIT_FP32_FRACTION_MASK) || !emit_test_ecx_ecx(w) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &fraction_zero_disp) || !emit_test_eax_imm32(w, RV64_JIT_FP32_QUIET_NAN_BIT) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &signalling_nan_disp) || !emit_fp_class_result_arm(w, 9, done_disps, &done_count))
    {
        return false;
    }
    ...
    if (!emit_test_eax_imm32(w, RV64_JIT_FP32_SIGN_MASK) || !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &positive_infinity_disp) ||
    ...
    if (!emit_mov_ecx_eax(w) || !emit_and_ecx_imm32(w, RV64_JIT_FP32_FRACTION_MASK) || !emit_test_ecx_ecx(w) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &zero_fraction_disp) || !emit_test_eax_imm32(w, RV64_JIT_FP32_SIGN_MASK) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &positive_subnormal_disp) || !emit_fp_class_result_arm(w, 2, done_disps, &done_count))
    {
        return false;
    }
    ...
    if (!emit_test_eax_imm32(w, RV64_JIT_FP32_SIGN_MASK) || !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &positive_zero_disp) ||
...
static bool emit_fp_classify_d_rax(rv64_jit_writer_t *w)
{
    ...
    if (!emit_mov_rcx_rax(w) || !emit_movabs_rdx(w, RV64_JIT_FP64_EXPONENT_MASK) || !emit_rcx_rdx_alu64(w, HOST_ALU_AND) || !emit_cmp_rcx_rdx(w) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &exponent_all_ones_disp) || !emit_test_rcx_rcx(w) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &exponent_zero_disp) || !emit_movabs_rdx(w, RV64_JIT_FP64_SIGN_MASK) || !emit_test_rax_rdx(w) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &positive_normal_disp) || !emit_fp_class_result_arm(w, 1, done_disps, &done_count))
    {
        return false;
    }
    ...
    patch_rel32(exponent_all_ones_disp, w->cur);
    if (!emit_mov_rcx_rax(w) || !emit_movabs_rdx(w, RV64_JIT_FP64_FRACTION_MASK) || !emit_rcx_rdx_alu64(w, HOST_ALU_AND) || !emit_test_rcx_rcx(w) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &fraction_zero_disp) || !emit_movabs_rdx(w, RV64_JIT_FP64_QUIET_NAN_BIT) ||
        !emit_test_rax_rdx(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &signalling_nan_disp) ||
    ...
    patch_rel32(fraction_zero_disp, w->cur);
    if (!emit_movabs_rdx(w, RV64_JIT_FP64_SIGN_MASK) || !emit_test_rax_rdx(w) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &positive_infinity_disp) || !emit_fp_class_result_arm(w, 0, done_disps, &done_count))
    {
        return false;
    }
    ...
    patch_rel32(exponent_zero_disp, w->cur);
    if (!emit_mov_rcx_rax(w) || !emit_movabs_rdx(w, RV64_JIT_FP64_FRACTION_MASK) || !emit_rcx_rdx_alu64(w, HOST_ALU_AND) || !emit_test_rcx_rcx(w) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &zero_fraction_disp) || !emit_movabs_rdx(w, RV64_JIT_FP64_SIGN_MASK) || !emit_test_rax_rdx(w) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &positive_subnormal_disp) || !emit_fp_class_result_arm(w, 2, done_disps, &done_count))
    {
        return false;
    }
    ...
    patch_rel32(zero_fraction_disp, w->cur);
    if (!emit_movabs_rdx(w, RV64_JIT_FP64_SIGN_MASK) || !emit_test_rax_rdx(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &positive_zero_disp) ||
        !emit_fp_class_result_arm(w, 3, done_disps, &done_count))
    {
        return false;
    }
...
static bool emit_fp_sign_s(rv64_jit_writer_t *w, rv64_jit_fp_exact_op_t op, uint32_t rd, uint32_t rs1, uint32_t rs2)
{
    if (!emit_load_cpu_u64_reg(w, HOST_REG_RAX, jit_fpr_offset(rs1)) || !emit_fp_unbox_s_rax(w) || !emit_mov_ecx_eax(w) ||
        !emit_load_cpu_u64_reg(w, HOST_REG_RAX, jit_fpr_offset(rs2)) || !emit_fp_unbox_s_rax(w))
    {
        return false;
    }
    ...
    case RV64_JIT_FP_EXACT_FSGNJ_S:
        if (!emit_eax_ecx_alu32(w, HOST_ALU_XOR) || !emit_and_eax_imm32(w, RV64_JIT_FP32_SIGN_MASK) || !emit_eax_ecx_alu32(w, HOST_ALU_XOR))
        {
            return false;
        }
        break;
    case RV64_JIT_FP_EXACT_FSGNJN_S:
        if (!emit_eax_ecx_alu32(w, HOST_ALU_XOR) || !emit_xor_eax_imm32(w, RV64_JIT_FP32_SIGN_MASK) ||
            !emit_and_eax_imm32(w, RV64_JIT_FP32_SIGN_MASK) || !emit_eax_ecx_alu32(w, HOST_ALU_XOR))
        {
            return false;
        }
        break;
    case RV64_JIT_FP_EXACT_FSGNJX_S:
        if (!emit_and_eax_imm32(w, RV64_JIT_FP32_SIGN_MASK) || !emit_eax_ecx_alu32(w, HOST_ALU_XOR))
        {
            return false;
        }
        break;
    ...
    return emit_movabs_rdx(w, RV64_JIT_FP32_BOX_MASK) && emit_rax_rdx_alu64(w, HOST_ALU_OR) &&
           emit_store_cpu_u64_reg(w, jit_fpr_offset(rd), HOST_REG_RAX) && emit_mark_fp_dirty(w);
}

/* Lower one D sign-injection instruction as a raw 64-bit bit operation. */
static bool emit_fp_sign_d(rv64_jit_writer_t *w, rv64_jit_fp_exact_op_t op, uint32_t rd, uint32_t rs1, uint32_t rs2)
{
    if (!emit_load_cpu_u64_reg(w, HOST_REG_RCX, jit_fpr_offset(rs1)) || !emit_load_cpu_u64_reg(w, HOST_REG_RAX, jit_fpr_offset(rs2)) ||
    ...
    case RV64_JIT_FP_EXACT_FSGNJ_D:
        if (!emit_rax_rcx_alu64(w, HOST_ALU_XOR) || !emit_rax_rdx_alu64(w, HOST_ALU_AND) || !emit_rax_rcx_alu64(w, HOST_ALU_XOR))
        {
            return false;
        }
        break;
    case RV64_JIT_FP_EXACT_FSGNJN_D:
        if (!emit_rax_rcx_alu64(w, HOST_ALU_XOR) || !emit_rax_rdx_alu64(w, HOST_ALU_XOR) || !emit_rax_rdx_alu64(w, HOST_ALU_AND) ||
    ...
    case RV64_JIT_FP_EXACT_FSGNJX_D:
        if (!emit_rax_rdx_alu64(w, HOST_ALU_AND) || !emit_rax_rcx_alu64(w, HOST_ALU_XOR))
        {
            return false;
        }
        break;
    ...
    return emit_store_cpu_u64_reg(w, jit_fpr_offset(rd), HOST_REG_RAX) && emit_mark_fp_dirty(w);
}

/* Emit the successful body of one validated exact-FP operation. */
static bool emit_native_fp_exact_body(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr, rv64_jit_fp_exact_op_t op)
{
    ...
    case RV64_JIT_FP_EXACT_FMV_X_W:
        return emit_load_cpu_u32_reg(w, HOST_REG_RAX, jit_fpr_offset(rs1)) && emit_cdqe(w) && jit_reg_write_rax(w, regs, rd);
    case RV64_JIT_FP_EXACT_FMV_W_X:
        return jit_reg_read_rax(w, regs, rs1) && emit_movabs_rdx(w, RV64_JIT_FP32_BOX_MASK) && emit_rax_rdx_alu64(w, HOST_ALU_OR) &&
               emit_store_cpu_u64_reg(w, jit_fpr_offset(rd), HOST_REG_RAX) && emit_mark_fp_dirty(w);
    case RV64_JIT_FP_EXACT_FMV_X_D:
        return emit_load_cpu_u64_reg(w, HOST_REG_RAX, jit_fpr_offset(rs1)) && jit_reg_write_rax(w, regs, rd);
    case RV64_JIT_FP_EXACT_FMV_D_X:
        return jit_reg_read_rax(w, regs, rs1) && emit_store_cpu_u64_reg(w, jit_fpr_offset(rd), HOST_REG_RAX) && emit_mark_fp_dirty(w);
    case RV64_JIT_FP_EXACT_FSGNJ_S:
    case RV64_JIT_FP_EXACT_FSGNJN_S:
    case RV64_JIT_FP_EXACT_FSGNJX_S:
    ...
    case RV64_JIT_FP_EXACT_FCLASS_S:
        return emit_load_cpu_u64_reg(w, HOST_REG_RAX, jit_fpr_offset(rs1)) && emit_fp_unbox_s_rax(w) && emit_fp_classify_s_rax(w) &&
               jit_reg_write_rax(w, regs, rd);
    case RV64_JIT_FP_EXACT_FCLASS_D:
        return emit_load_cpu_u64_reg(w, HOST_REG_RAX, jit_fpr_offset(rs1)) && emit_fp_classify_d_rax(w) && jit_reg_write_rax(w, regs, rd);
    default:
        return false;
    }
}
 ...
 */
static bool emit_native_fp_exact(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr, vaddr_t pc, uint32_t completed_count,
                                 rv64_jit_fp_exact_op_t op)
{
    ...
    if (!emit_test_cpu_u64_imm32(w, jit_mstatus_offset(), (uint32_t)RISCV_MSTATUS_FS_MASK) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &fs_off_disp) || !emit_native_fp_exact_body(w, regs, instr, op) ||
        !emit_inc_jit_stat_counter(w, &rv64_jit_stats.native_fp_exact_executions[op]) || !emit_jmp_rel32_placeholder(w, &done_disp))
    {
        return false;
    }

    patch_rel32(fs_off_disp, w->cur);
    if (!emit_interpreter_side_exit(w, &side_exit_regs, pc, completed_count, RV64_JIT_SIDE_EXIT_FP_FS_OFF))
    {
        return false;
    }
...
static bool emit_native_fp_memory_bare_load(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr, vaddr_t pc, uint32_t completed_count,
                                            rv64_jit_fp_memory_op_t op)
{
    const uint32_t rd = rv64_instr_rd(instr);
    const uint32_t rs1 = rv64_instr_rs1(instr);
    const int32_t imm = (int32_t)imm_i(instr);
    const uint32_t len = op == RV64_JIT_FP_MEMORY_FLW ? 4u : 8u;
    const uint32_t host_load_funct3 = op == RV64_JIT_FP_MEMORY_FLW ? RV64_FUNCT3_LWU : RV64_FUNCT3_LD;
     ...
     */
    if (!emit_test_cpu_u64_imm32(w, jit_mstatus_offset(), (uint32_t)RISCV_MSTATUS_FS_MASK) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &fs_off_disp) || !jit_reg_read_rax(w, regs, rs1) || !emit_add_rax_imm32(w, imm))
    {
        return false;
    }
    ...
    if (!emit_alignment_guard_al(w, len, &align_slow_disp) || !emit_mov_rdx_rax(w) || !emit_guard_bare_address_in_pmem(w, len, &range_slow_disp) ||
        !emit_direct_pmem_load_rax(w, host_load_funct3))
    {
        return false;
    }

    if (op == RV64_JIT_FP_MEMORY_FLW && (!emit_movabs_rdx(w, RV64_JIT_FP32_BOX_MASK) || !emit_rax_rdx_alu64(w, HOST_ALU_OR)))
    {
        return false;
    }

    if (!emit_store_cpu_u64_reg(w, jit_fpr_offset(rd), HOST_REG_RAX) || !emit_mark_fp_dirty(w) ||
        !emit_inc_jit_stat_counter(w, &rv64_jit_stats.native_fp_memory_executions[op]) || !emit_jmp_rel32_placeholder(w, &done_disp))
    {
        return false;
    }

    patch_rel32(range_slow_disp, w->cur);
    if (!emit_fp_memory_helper_terminal(w, regs, instr, pc, completed_count))
    {
        return false;
    }
    ...
    patch_rel32(fs_off_disp, w->cur);
    if (!emit_interpreter_side_exit(w, &fs_off_regs, pc, completed_count, RV64_JIT_SIDE_EXIT_FP_FS_OFF))
    {
        return false;
    }
...
static bool emit_native_fp_memory_bare_store(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr, vaddr_t pc, uint32_t completed_count,
                                             rv64_jit_fp_memory_op_t op)
{
    const uint32_t rs1 = rv64_instr_rs1(instr);
    const uint32_t rs2 = rv64_instr_rs2(instr);
    const int32_t imm = (int32_t)imm_s(instr);
    const uint32_t len = op == RV64_JIT_FP_MEMORY_FSW ? 4u : 8u;
    rv64_jit_reg_cache_t fs_off_regs = *regs;
    rv64_jit_reg_cache_t side_exit_regs;
    uint8_t *fs_off_disp = NULL;
    uint8_t *align_slow_disp = NULL;
    ...
    if (!emit_alignment_guard_al(w, len, &align_slow_disp) || !emit_mov_rdx_rax(w) || !emit_guard_bare_address_in_pmem(w, len, &range_slow_disp) ||
         ...
         */
        !emit_load_cpu_u64_reg(w, HOST_REG_RCX, jit_fpr_offset(rs2)) ||
        !emit_guard_store_not_compiled_source(w, len, &cross_chunk_disp, &source_chunk_disp) ||
        !emit_guard_store_not_translation_dependency(w, &data_page_table_disp, &ifetch_page_table_disp) || !emit_direct_pmem_store_from_rcx(w, len) ||
        !emit_inc_jit_stat_counter(w, &rv64_jit_stats.native_fp_memory_executions[op]) || !emit_jmp_rel32_placeholder(w, &done_disp))
    {
        return false;
    }
    ...
    if (!emit_interpreter_side_exit(w, &side_exit_regs, pc, completed_count, RV64_JIT_SIDE_EXIT_STORE_SOURCE))
    {
        return false;
    }
...
static bool emit_native_fp_memory_bare(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr, vaddr_t pc, uint32_t completed_count,
                                       rv64_jit_fp_memory_op_t op)
{
    switch (op)
    {
    ...
    case RV64_JIT_FP_MEMORY_FLD:
        return emit_native_fp_memory_bare_load(w, regs, instr, pc, completed_count, op);
    case RV64_JIT_FP_MEMORY_FSW:
    case RV64_JIT_FP_MEMORY_FSD:
        return emit_native_fp_memory_bare_store(w, regs, instr, pc, completed_count, op);
    default:
        return false;
    }
}
                                           ...
                                           bool is_store, vaddr_t pc, uint32_t completed_count, rv64_jit_side_exit_reason_t reason)
{
    ...
    return emit_interpreter_side_exit(w, side_exit_regs, pc, completed_count, reason);
}

/* Emit a translated FLW or FLD for an already-filled inline data-TLB entry. */
static bool emit_native_fp_memory_paged_load(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr, vaddr_t pc, uint32_t completed_count,
                                             rv64_jit_fp_memory_op_t op)
{
    const uint32_t rd = rv64_instr_rd(instr);
    const uint32_t rs1 = rv64_instr_rs1(instr);
    ...
    if (!emit_alignment_guard_al(w, len, &align_slow_disp) ||
         ...
         * RAX.
         */
        !emit_mov_rcx_rax(w) || !emit_inline_sv39_load_fast_path(w, host_load_funct3, len, &tlb_guard))
    {
        return false;
    }
    ...
    if (!emit_fp_memory_probe_side_exit(w, &side_exit_regs, true, len, false, pc, completed_count, RV64_JIT_SIDE_EXIT_LOAD_GUARD))
    {
        return false;
    }
...
static bool emit_native_fp_memory_paged_store(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr, vaddr_t pc, uint32_t completed_count,
                                              rv64_jit_fp_memory_op_t op)
{
    const uint32_t rs1 = rv64_instr_rs1(instr);
    const uint32_t rs2 = rv64_instr_rs2(instr);
    ...
    if (!emit_alignment_guard_al(w, len, &align_slow_disp) ||
         ...
         */
        !emit_mov_rdi_rax(w) || !emit_load_cpu_u64_reg(w, HOST_REG_RCX, jit_fpr_offset(rs2)) || !emit_inline_sv39_store_address(w, len, &tlb_guard) ||
        !emit_guard_store_not_compiled_source(w, len, &cross_chunk_disp, &source_chunk_disp) ||
        ...
        !emit_direct_pmem_store_from_rcx(w, len) || !emit_inc_jit_stat_counter(w, &rv64_jit_stats.native_fp_memory_executions[op]) ||
    ...
    const uint8_t *translation_slow_path = w->cur;
    patch_tlb_guard(&tlb_guard, translation_slow_path);
    if (!emit_fp_memory_probe_side_exit(w, &side_exit_regs, false, len, true, pc, completed_count, RV64_JIT_SIDE_EXIT_STORE_GUARD))
    {
        return false;
    }
...
static bool emit_native_fp_memory_paged(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr, vaddr_t pc, uint32_t completed_count,
                                        rv64_jit_fp_memory_op_t op)
{
    switch (op)
    {
    case RV64_JIT_FP_MEMORY_FLW:
    case RV64_JIT_FP_MEMORY_FLD:
        return emit_native_fp_memory_paged_load(w, regs, instr, pc, completed_count, op);
    case RV64_JIT_FP_MEMORY_FSW:
    case RV64_JIT_FP_MEMORY_FSD:
        return emit_native_fp_memory_paged_store(w, regs, instr, pc, completed_count, op);
    default:
        return false;
    }
}
...
bool rv64_jit_emit_fp_instr(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr, vaddr_t pc, uint32_t completed_count, bool *ends_block)
{
    ...
    *ends_block = false;
    ...
    {
        if (!emit_native_fp_exact(w, regs, instr, pc, completed_count, exact_op))
        {
            return false;
        }
    ...
    const riscv_fpu_gpr_effect_t gpr_effect = riscv_fpu_gpr_effect(instr);
    const bool defer_gpr_sync = !is_memory && jit_reg_can_defer_fp_helper_sync(gpr_effect);
    ...
    if (defer_gpr_sync)
    {
        ...
        Assert(regs->slot_count == RV64_JIT_HREG_BASE_COUNT, "jit: deferred FP helper sync saw an R8 cache slot");
        if (!jit_reg_flush_mask(w, regs, gpr_effect.read_mask, &input_flushes))
        {
            return false;
        }
    }
    ...
    else
    {
        ...
        if (defer_gpr_sync)
        {
            jit_reg_apply_fp_helper_effect(regs, gpr_effect, input_flushes, trap_stores);
        }
        else
        {
            rv64_jit_reg_cache_init(regs);
...
static bool emit_direct_link_miss_jcc(rv64_jit_writer_t *w, uint8_t jcc_opcode, uint8_t **miss_disps, uint32_t *miss_count)
{
    Assert(*miss_count < RV64_JIT_DIRECT_LINK_MAX_MISS_PATCHES, "jit: RV64 direct-link miss patch list overflow");
...
static bool emit_link_guard_miss_jcc(rv64_jit_writer_t *w, uint8_t jcc_opcode, rv64_jit_link_guard_fixups_t *fixups)
{
    return emit_direct_link_miss_jcc(w, jcc_opcode, fixups->disps, &fixups->count);
}

static void patch_link_guard_misses(const rv64_jit_link_guard_fixups_t *fixups, const uint8_t *miss_path)
{
...
static bool emit_link_target_guards(rv64_jit_writer_t *w, const rv64_jit_link_candidate_t *candidate, const rv64_jit_link_guard_config_t *config,
                                    rv64_jit_link_guard_fixups_t *fixups);

#if RV64_JIT_STATS
static bool emit_guarded_link_taken_stats(rv64_jit_writer_t *w, const rv64_jit_link_candidate_t *candidate);
#endif

/* Emit the conservative block exit used when cross-block direct links are off. */
bool rv64_jit_emit_plain_block_exit(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, vaddr_t target_pc, uint32_t completed_count)
{
    return jit_reg_emit_flush_all_dirty(w, regs) && emit_store_pc_imm(w, target_pc) && emit_return_total_retired(w, completed_count);
}
 ...
 */
static bool emit_accumulate_loop_extra(rv64_jit_writer_t *w, uint32_t completed_count)
{
    return emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv64_jit_loop_extra) && emit_mov_eax_m32_rdx(w) && emit_add_eax_imm32(w, completed_count) &&
...
static bool emit_chain_accepted_stats(rv64_jit_writer_t *w)
{
    ...
    if (!emit_test_esi_esi(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &direct_disp) ||
        !emit_cmp_esi_imm8(w, RV64_JIT_CHAIN_STATS_RETURN_SECONDARY) || !emit_jcc_rel32_placeholder(w, HOST_JCC_A, &jalr_disp) ||
        !emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_pic_patched_entries[RV64_JIT_INDIRECT_PIC_RETURN]) ||
        !emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_pic_hits[RV64_JIT_INDIRECT_PIC_RETURN]) ||
        !emit_cmp_esi_imm8(w, RV64_JIT_CHAIN_STATS_RETURN_SECONDARY) || !emit_jcc_rel32_placeholder(w, HOST_JCC_NE, &return_primary_disp) ||
        !emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_pic_secondary_hits[RV64_JIT_INDIRECT_PIC_RETURN]))
    {
        return false;
    }
    ...
    if (!emit_inc_jit_stat_counter(w, &rv64_jit_stats.direct_return_link_taken_count) || !emit_jmp_rel32_placeholder(w, &return_done_disp))
    {
        return false;
    }
    ...
    if (!emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_pic_patched_entries[RV64_JIT_INDIRECT_PIC_JALR]) ||
        !emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_pic_hits[RV64_JIT_INDIRECT_PIC_JALR]) ||
        !emit_cmp_esi_imm8(w, RV64_JIT_CHAIN_STATS_JALR_SECONDARY) || !emit_jcc_rel32_placeholder(w, HOST_JCC_NE, &jalr_primary_disp) ||
        !emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_pic_secondary_hits[RV64_JIT_INDIRECT_PIC_JALR]))
    {
        return false;
    }
    ...
    if (!emit_inc_jit_stat_counter(w, &rv64_jit_stats.direct_jalr_link_taken_count) || !emit_jmp_rel32_placeholder(w, &jalr_done_disp))
    {
        return false;
    }
    ...
    if (!emit_inc_jit_stat_counter(w, &rv64_jit_stats.patched_direct_link_taken_count) || !emit_test_rdi_rdi(w) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &direct_done_disp) || !emit_inc_m64_rdi(w))
    {
        return false;
    }
...
static bool emit_chain_rejected_stats(rv64_jit_writer_t *w)
{
    ...
    if (!emit_test_esi_esi(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &done_disp) ||
        !emit_cmp_esi_imm8(w, RV64_JIT_CHAIN_STATS_RETURN_SECONDARY) || !emit_jcc_rel32_placeholder(w, HOST_JCC_A, &jalr_disp) ||
        !emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_pic_budget_rejections[RV64_JIT_INDIRECT_PIC_RETURN]) ||
        !emit_inc_jit_stat_counter(w, &rv64_jit_stats.direct_return_link_miss_count) || !emit_jmp_rel32_placeholder(w, &return_done_disp))
    {
        return false;
    }
    ...
    if (!emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_pic_budget_rejections[RV64_JIT_INDIRECT_PIC_JALR]) ||
        !emit_inc_jit_stat_counter(w, &rv64_jit_stats.direct_jalr_link_miss_count))
    {
        return false;
    }
 ...
 */
bool rv64_jit_emit_chain_entry(rv64_jit_writer_t *w, vaddr_t pc, uint32_t insn_count, const uint8_t *body_entry, const uint8_t **chain_entry)
{
    ...
    *chain_entry = w->cur;
    ...
    if (!emit_mov_ecx_eax(w) || !emit_add_ecx_imm32(w, insn_count) || !emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv64_jit_entry_budget) ||
        !emit_cmp_ecx_m32_rdx(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_A, &over_budget_disp) ||
#if RV64_JIT_STATS
    ...
    return emit_store_pc_imm(w, pc) && emit_inc_jit_stat_counter(w, &rv64_jit_stats.direct_link_miss_count) && emit_return_total_retired(w, 0);
}
 ...
 */
static bool emit_guarded_direct_link_exit(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, vaddr_t target_pc, uint32_t completed_count,
                                          bool source_uses_data_state, bool registers_already_flushed, bool count_already_accumulated,
                                          uint64_t *extra_taken_counter)
{
    const word_t satp = cpu.csr.satp;
    const uint32_t ifetch_state = rv64_jit_ifetch_state();
    rv64_jit_block_t *target = rv64_jit_cache_slot_context(target_pc, satp, ifetch_state);
    const uint32_t body_entry_off = (uint32_t)offsetof(rv64_jit_block_t, body_entry);
    ...
    if ((!registers_already_flushed && !jit_reg_emit_flush_all_dirty(w, regs)) ||
        !emit_link_target_guards(w, &candidate, &guard_config, &guard_fixups))
    {
        return false;
    }
    ...
    if (!emit_inc_jit_stat_counter(w, &rv64_jit_stats.direct_link_taken_count) ||
        !(extra_taken_counter == NULL || emit_inc_jit_stat_counter(w, extra_taken_counter)) || !emit_movabs_rdx(w, (uint64_t)(uintptr_t)target) ||
        !emit_mov_rax_rdxq_field(w, body_entry_off) || !emit_jmp_rax(w))
    {
        return false;
    }
    ...
    return emit_store_pc_imm(w, target_pc) && emit_inc_jit_stat_counter(w, &rv64_jit_stats.direct_link_miss_count) &&
           emit_return_total_retired(w, count_already_accumulated ? 0 : completed_count);
}
 ...
 */
static bool emit_patchable_direct_link_exit(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, vaddr_t target_pc, uint32_t completed_count,
                                            bool source_uses_data_state, uint64_t *extra_taken_counter)
{
    Assert(w->links != NULL, "jit: missing RV64 direct-link builder");
    Assert(w->links->state.count < RV64_JIT_BLOCK_MAX_LINKS, "jit: RV64 direct-link builder overflow");
    ...
    if (!jit_reg_emit_flush_all_dirty(w, regs) || !emit_accumulate_loop_extra(w, completed_count) || !emit_jmp_rel32_placeholder(w, &selector_disp))
    {
        return false;
    }
    ...
    if (!emit_zero_esi(w) || !(extra_taken_counter != NULL ? emit_movabs_rdi(w, (uint64_t)(uintptr_t)extra_taken_counter) : emit_zero_edi(w)) ||
    ...
    if (!emit_guarded_direct_link_exit(w, regs, target_pc, completed_count, source_uses_data_state, true, true, extra_taken_counter))
    {
        return false;
    }
    ...
    rv64_jit_link_build_record_t *record = &w->links->records[w->links->state.count++];
                                    ...
                                    bool source_uses_data_state, uint64_t *extra_taken_counter)
{
    ...
    {
        return emit_patchable_direct_link_exit(w, regs, target_pc, completed_count, source_uses_data_state, extra_taken_counter);
    }

    return emit_guarded_direct_link_exit(w, regs, target_pc, completed_count, source_uses_data_state, false, false, extra_taken_counter);
}

/* Reserve the complete R8-pointed target, then commit this source's count. */
static bool emit_indirect_target_budget(rv64_jit_writer_t *w, uint32_t completed_count, uint8_t **over_budget_disp)
{
    const uint32_t insn_count_off = (uint32_t)offsetof(rv64_jit_block_t, insn_count);

    return emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv64_jit_loop_extra) && emit_mov_eax_m32_rdx(w) && emit_add_eax_imm32(w, completed_count) &&
           emit_mov_ecx_eax(w) && emit_add_ecx_r8d_field(w, insn_count_off) && emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv64_jit_entry_budget) &&
           emit_cmp_ecx_m32_rdx(w) && emit_jcc_rel32_placeholder(w, HOST_JCC_A, over_budget_disp) &&
           emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv64_jit_loop_extra) && emit_mov_m32_rdx_eax(w);
}

/* Materialise the cache slot while preserving each path's established ABI. */
static bool emit_link_candidate_address(rv64_jit_writer_t *w, const rv64_jit_link_candidate_t *candidate)
{
    switch (candidate->kind)
    {
    case RV64_JIT_LINK_CANDIDATE_KNOWN_RDX:
        Assert(candidate->known_slot != NULL, "jit: missing known RV64 direct-link slot");
        return emit_movabs_rdx(w, (uint64_t)(uintptr_t)candidate->known_slot);
    case RV64_JIT_LINK_CANDIDATE_RUNTIME_R8:
        /* Match rv64_jit_cache_hash_context() using the target in R9. */
        return emit_mov_rdx_r9(w) && emit_shr_rdx_imm(w, RV64_JIT_CACHE_PC_SHIFT) && emit_xor_edx_imm32(w, candidate->context_mix) &&
               emit_and_edx_imm32(w, RV64_JIT_CACHE_SIZE - 1u) && emit_imul_rdx_imm32(w, (uint32_t)sizeof(rv64_jit_block_t)) &&
               emit_movabs_reg(w, HOST_REG_R8, (uint64_t)(uintptr_t)rv64_jit_cache) && emit_add_r8_rdx(w);
    default:
        Assert(false, "jit: invalid RV64 direct-link candidate kind %u", (unsigned)candidate->kind);
        return false;
    }
}

static bool emit_link_candidate_cmp_byte_imm(rv64_jit_writer_t *w, const rv64_jit_link_candidate_t *candidate, uint32_t offset, uint8_t value)
{
    ...
    Assert(candidate->kind == RV64_JIT_LINK_CANDIDATE_RUNTIME_R8, "jit: invalid RV64 byte-guard candidate kind %u", (unsigned)candidate->kind);
    return emit_cmp_r8b_field_imm8(w, offset, value);
}

static bool emit_link_candidate_cmp_dword_imm(rv64_jit_writer_t *w, const rv64_jit_link_candidate_t *candidate, uint32_t offset, uint32_t value)
{
    if (candidate->kind == RV64_JIT_LINK_CANDIDATE_KNOWN_RDX)
    {
    ...
    Assert(candidate->kind == RV64_JIT_LINK_CANDIDATE_RUNTIME_R8, "jit: invalid RV64 dword-guard candidate kind %u", (unsigned)candidate->kind);
    return emit_cmp_r8d_field_imm32(w, offset, value);
}

static bool emit_link_candidate_cmp_qword_rax(rv64_jit_writer_t *w, const rv64_jit_link_candidate_t *candidate, uint32_t offset)
{
    if (candidate->kind == RV64_JIT_LINK_CANDIDATE_KNOWN_RDX)
    {
    ...
    Assert(candidate->kind == RV64_JIT_LINK_CANDIDATE_RUNTIME_R8, "jit: invalid RV64 qword-guard candidate kind %u", (unsigned)candidate->kind);
    return emit_cmp_r8q_field_rax(w, offset);
}

static bool emit_link_candidate_cmp_qword_imm8(rv64_jit_writer_t *w, const rv64_jit_link_candidate_t *candidate, uint32_t offset, uint8_t value)
{
    if (candidate->kind == RV64_JIT_LINK_CANDIDATE_KNOWN_RDX)
    {
...
static bool emit_link_candidate_pc(rv64_jit_writer_t *w, const rv64_jit_link_candidate_t *candidate, uint32_t pc_offset)
{
    if (candidate->kind == RV64_JIT_LINK_CANDIDATE_KNOWN_RDX)
    {
        return emit_movabs_rax(w, candidate->target_pc) && emit_cmp_rdxq_field_rax(w, pc_offset);
    }

    Assert(candidate->kind == RV64_JIT_LINK_CANDIDATE_RUNTIME_R8, "jit: invalid RV64 PC-guard candidate kind %u", (unsigned)candidate->kind);
    return emit_cmp_r8q_field_r9(w, pc_offset);
}

static bool emit_link_candidate_satp(rv64_jit_writer_t *w, const rv64_jit_link_candidate_t *candidate, uint32_t satp_offset, word_t satp)
{
    if (candidate->kind == RV64_JIT_LINK_CANDIDATE_KNOWN_RDX)
    {
        return emit_movabs_rax(w, satp) && emit_cmp_rdxq_field_rax(w, satp_offset);
    }

    Assert(candidate->kind == RV64_JIT_LINK_CANDIDATE_RUNTIME_R8, "jit: invalid RV64 satp-guard candidate kind %u", (unsigned)candidate->kind);
    return emit_movabs_rdx(w, satp) && emit_cmp_r8q_field_rdx(w, satp_offset);
}

/* Emit the path-specific form of the final whole-target budget guard. */
static bool emit_link_target_budget_guard(rv64_jit_writer_t *w, const rv64_jit_link_candidate_t *candidate,
                                          const rv64_jit_link_guard_config_t *config, rv64_jit_link_guard_fixups_t *fixups)
{
    if (candidate->kind == RV64_JIT_LINK_CANDIDATE_KNOWN_RDX)
    {
        const uint32_t insn_count_off = (uint32_t)offsetof(rv64_jit_block_t, insn_count);

        if (!emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv64_jit_loop_extra) || !emit_mov_eax_m32_rdx(w))
        {
            return false;
        }
         ...
         */
        if (!config->count_already_accumulated && !emit_add_eax_imm32(w, config->completed_count))
        {
            return false;
        }

        if (!emit_mov_ecx_eax(w) || !emit_movabs_rdx(w, (uint64_t)(uintptr_t)candidate->known_slot) || !emit_add_ecx_rdxd_field(w, insn_count_off) ||
            !emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv64_jit_entry_budget) || !emit_cmp_ecx_m32_rdx(w) ||
        ...
        if (!config->count_already_accumulated && (!emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv64_jit_loop_extra) || !emit_mov_m32_rdx_eax(w)))
        {
            return false;
        }
    ...
    Assert(candidate->kind == RV64_JIT_LINK_CANDIDATE_RUNTIME_R8, "jit: invalid RV64 budget-guard candidate kind %u", (unsigned)candidate->kind);
    Assert(!config->count_already_accumulated, "jit: runtime RV64 direct link received an accumulated count");

    uint8_t *over_budget_disp = NULL;
    if (!emit_indirect_target_budget(w, config->completed_count, &over_budget_disp))
    {
        return false;
    }

    Assert(fixups->count < RV64_JIT_DIRECT_LINK_MAX_MISS_PATCHES, "jit: runtime-target budget guard list overflow");
                                    ...
                                    rv64_jit_link_guard_fixups_t *fixups)
{
    ...
    const uint32_t uses_data_state_off = (uint32_t)offsetof(rv64_jit_block_t, uses_data_state);
    ...
    const uint32_t ifetch_state_off = (uint32_t)offsetof(rv64_jit_block_t, ifetch_state);
    const uint32_t data_state_off = (uint32_t)offsetof(rv64_jit_block_t, data_state);
    const uint32_t ifetch_generation_off = (uint32_t)offsetof(rv64_jit_block_t, ifetch_generation);
    const uint32_t body_entry_off = (uint32_t)offsetof(rv64_jit_block_t, body_entry);
    ...
    *fixups = (rv64_jit_link_guard_fixups_t){0};
    ...
    if (!emit_link_candidate_address(w, candidate) || !emit_link_candidate_cmp_byte_imm(w, candidate, valid_off, 1) ||
        !emit_link_guard_miss_jcc(w, HOST_JCC_NE, fixups) || !emit_link_candidate_pc(w, candidate, pc_off) ||
        !emit_link_guard_miss_jcc(w, HOST_JCC_NE, fixups) || !emit_link_candidate_satp(w, candidate, satp_off, config->satp) ||
        !emit_link_guard_miss_jcc(w, HOST_JCC_NE, fixups) ||
        !emit_link_candidate_cmp_dword_imm(w, candidate, ifetch_state_off, config->ifetch_state) ||
        !emit_link_guard_miss_jcc(w, HOST_JCC_NE, fixups) || !emit_link_candidate_cmp_byte_imm(w, candidate, uses_data_state_off, 0))
    {
        return false;
    }
    ...
    if (config->source_uses_data_state)
    {
        ...
        if (!emit_jcc_rel32_placeholder(w, HOST_JCC_E, &data_state_ok_disp) ||
            !emit_link_candidate_cmp_dword_imm(w, candidate, data_state_off, config->data_state) || !emit_link_guard_miss_jcc(w, HOST_JCC_NE, fixups))
        {
            return false;
        }
     ...
     */
    if (!emit_link_candidate_cmp_byte_imm(w, candidate, translated_off, 0) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &ifetch_generation_ok_disp) ||
        !emit_movabs_rax(w, (uint64_t)(uintptr_t)&rv64_jit_ifetch_generation) || !emit_mov_rax_m64_rax(w) ||
        !emit_link_candidate_cmp_qword_rax(w, candidate, ifetch_generation_off) || !emit_link_guard_miss_jcc(w, HOST_JCC_NE, fixups))
    {
        return false;
    }
    ...
    if (!emit_link_candidate_cmp_qword_imm8(w, candidate, body_entry_off, 0) || !emit_link_guard_miss_jcc(w, HOST_JCC_E, fixups) ||
        !emit_link_target_budget_guard(w, candidate, config, fixups))
    {
        return false;
    }

    Assert(fixups->count == RV64_JIT_DIRECT_LINK_GUARD_COUNT, "jit: shared RV64 direct-link guard policy drifted");
    return true;
}
...
static bool emit_guarded_link_taken_stats(rv64_jit_writer_t *w, const rv64_jit_link_candidate_t *candidate)
{
    ...
    if ((candidate->kind == RV64_JIT_LINK_CANDIDATE_KNOWN_RDX && !emit_link_candidate_address(w, candidate)) ||
        !emit_link_candidate_cmp_byte_imm(w, candidate, translated_off, 0) || !emit_jcc_rel32_placeholder(w, HOST_JCC_NE, &guarded_taken_disp) ||
        !emit_link_candidate_cmp_byte_imm(w, candidate, uses_data_state_off, 0) || !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &guarded_done_disp))
    {
        return false;
    }
 ...
 */
static bool emit_indirect_pic_way_probe(rv64_jit_writer_t *w, uint32_t way, rv64_jit_indirect_pic_kind_t pic_kind,
                                        rv64_jit_indirect_pic_way_fixups_t *fixups)
{
    Assert(w->indirect_pic != NULL && w->indirect_pic->state.used, "jit: missing active RV64 indirect PIC builder");
    Assert(way < RV64_JIT_INDIRECT_PIC_WAYS, "jit: invalid RV64 indirect PIC way %u", way);
    Assert(pic_kind < RV64_JIT_INDIRECT_PIC_KIND_COUNT, "jit: invalid RV64 indirect PIC kind %u", pic_kind);
    Assert(fixups != NULL, "jit: missing RV64 indirect PIC way fixups");

    const uint32_t way_base = (uint32_t)offsetof(rv64_jit_indirect_pic_t, ways) + way * (uint32_t)sizeof(rv64_jit_indirect_pic_entry_t);
    const uint32_t target_pc_off = (uint32_t)offsetof(rv64_jit_indirect_pic_entry_t, target_pc);
    const uint32_t target_generation_off = (uint32_t)offsetof(rv64_jit_indirect_pic_entry_t, target_generation);
    const uint32_t target_slot_off = (uint32_t)offsetof(rv64_jit_indirect_pic_entry_t, target_slot);
    const uint32_t valid_off = (uint32_t)offsetof(rv64_jit_block_t, valid);
    const uint32_t generation_off = (uint32_t)offsetof(rv64_jit_block_t, generation);
    uint8_t *stale_disp = NULL;

    *fixups = (rv64_jit_indirect_pic_way_fixups_t){0};

    if (!emit_cmp_rdxq_field_r9(w, way_base + target_pc_off) || !emit_jcc_rel32_placeholder(w, HOST_JCC_NE, &fixups->tag_miss_disp) ||
        !emit_jmp_rel32_placeholder(w, &w->indirect_pic->selector_disps[way]))
    {
        return false;
    }
    ...
    if (!emit_mov_r8_rdxq_field(w, way_base + target_slot_off) || !emit_test_r8_r8(w) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &fixups->empty_disp) || !emit_cmp_r8b_field_imm8(w, valid_off, 1) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_NE, &stale_disp) || !emit_mov_rax_rdxq_field(w, way_base + target_generation_off) ||
        !emit_cmp_r8q_field_rax(w, generation_off) || !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &fixups->hit_disp))
    {
        return false;
    }
    ...
    return emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_pic_stale_rejections[pic_kind]) &&
           emit_jmp_rel32_placeholder(w, &fixups->stale_to_miss_disp);
}
 ...
 */
static bool emit_indirect_pic_probe(rv64_jit_writer_t *w, rv64_jit_indirect_pic_kind_t pic_kind, uint8_t *hit_disps[RV64_JIT_INDIRECT_PIC_WAYS])
{
    rv64_jit_indirect_pic_way_fixups_t way_fixups[RV64_JIT_INDIRECT_PIC_WAYS] = {0};
    ...
    for (uint32_t way = 0; way < RV64_JIT_INDIRECT_PIC_WAYS; way++)
    {
        ...
        if (!emit_indirect_pic_way_probe(w, way, pic_kind, &way_fixups[way]))
        {
            return false;
        }
    }
    ...
    return emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_pic_misses[pic_kind]);
}
 ...
 */
static bool emit_indirect_jump_cache_probe(rv64_jit_writer_t *w, uint32_t completed_count, uint64_t *subset_taken_counter,
                                           uint8_t **budget_to_miss_disp)
{
    const uint32_t cache_generation_off = (uint32_t)offsetof(rv64_jit_indirect_jump_cache_entry_t, target_generation);
    const uint32_t cache_slot_off = (uint32_t)offsetof(rv64_jit_indirect_jump_cache_entry_t, target_slot);
    const uint32_t valid_off = (uint32_t)offsetof(rv64_jit_block_t, valid);
    const uint32_t generation_off = (uint32_t)offsetof(rv64_jit_block_t, generation);
    const uint32_t pc_off = (uint32_t)offsetof(rv64_jit_block_t, pc);
    const uint32_t body_entry_off = (uint32_t)offsetof(rv64_jit_block_t, body_entry);
    ...
    Assert(w->indirect_jump_cache != NULL && w->indirect_jump_cache->state.used, "jit: missing active RV64 indirect jump cache builder");
    Assert(budget_to_miss_disp != NULL, "jit: missing RV64 jump-cache budget fixup result");
    *budget_to_miss_disp = NULL;
     ...
     */
    if (!emit_mov_rdx_r9(w) || !emit_and_edx_imm8(w, RV64_JIT_INDIRECT_JUMP_CACHE_BYTE_MASK) ||
        !emit_shl_edx_imm(w, RV64_JIT_INDIRECT_JUMP_CACHE_OFFSET_SHIFT) || !emit_movabs_indirect_jump_cache_address(w, HOST_REG_RDI) ||
        !emit_add_rdi_rdx(w) || !emit_mov_r8_rdiq_field(w, cache_slot_off) || !emit_test_r8_r8(w) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &empty_disp) || !emit_cmp_r8q_field_r9(w, pc_off) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_NE, &tag_miss_disp) || !emit_cmp_r8b_field_imm8(w, valid_off, 1) ||
        !emit_jcc_rel32_placeholder(w, HOST_JCC_NE, &invalid_stale_disp) || !emit_mov_rax_rdiq_field(w, cache_generation_off) ||
        !emit_test_rax_rax(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &zero_generation_stale_disp) ||
        !emit_cmp_r8q_field_rax(w, generation_off) || !emit_jcc_rel32_placeholder(w, HOST_JCC_NE, &changed_generation_stale_disp) ||
        !emit_indirect_target_budget(w, completed_count, &budget_disp) || !emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_jump_cache_hits) ||
        !emit_inc_jit_stat_counter(w, &rv64_jit_stats.direct_link_taken_count) ||
        !(subset_taken_counter == NULL || emit_inc_jit_stat_counter(w, subset_taken_counter)) || !emit_mov_rax_r8q_field(w, body_entry_off) ||
        !emit_jmp_rax(w))
    {
        return false;
    }
    ...
    if (!emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_jump_cache_budget_rejections) || !emit_jmp_rel32_placeholder(w, budget_to_miss_disp))
    {
        return false;
    }
    ...
    if (!emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_jump_cache_stale_rejections))
    {
        return false;
    }
    ...
    return emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_jump_cache_misses);
}
 ...
 */
static bool emit_indirect_authoritative_hit(rv64_jit_writer_t *w, bool pic_enabled, bool jump_cache_enabled, uint64_t *subset_taken_counter)
{
    const uint32_t generation_off = (uint32_t)offsetof(rv64_jit_block_t, generation);
    const uint32_t body_entry_off = (uint32_t)offsetof(rv64_jit_block_t, body_entry);

    Assert(!(pic_enabled && jump_cache_enabled), "jit: authoritative RV64 hit selected both indirect caches");
    ...
    if (pic_enabled)
    {
         ...
         */
        return emit_mov_rsi_r9(w) && emit_mov_rdx_r8(w) && emit_movabs_indirect_pic_address(w, HOST_REG_RDI) &&
               emit_call_abs(w, (uintptr_t)rv64_jit_indirect_pic_refill) && emit_mov_r9_rax(w) && emit_load_jit_bases(w) &&
               emit_inc_jit_stat_counter(w, &rv64_jit_stats.direct_link_taken_count) &&
               (subset_taken_counter == NULL || emit_inc_jit_stat_counter(w, subset_taken_counter)) && emit_jmp_r9(w);
    }
    ...
    if (jump_cache_enabled)
    {
        ...
        if (!emit_mov_rax_rdiq_field(w, cache_slot_off) || !emit_test_rax_rax(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &empty_entry_disp) ||
            !emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_jump_cache_replacements))
        {
            return false;
        }
         ...
         */
        if (!emit_zero_rax(w) || !emit_mov_rdiq_field_rax(w, cache_generation_off) || !emit_mov_rdiq_field_r8(w, cache_slot_off) ||
            !emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_jump_cache_fills) || !emit_mov_rax_r8q_field(w, generation_off) ||
    ...
    return emit_inc_jit_stat_counter(w, &rv64_jit_stats.direct_link_taken_count) &&
           (subset_taken_counter == NULL || emit_inc_jit_stat_counter(w, subset_taken_counter)) && emit_mov_rax_r8q_field(w, body_entry_off) &&
           emit_jmp_rax(w);
}
 ...
 */
static bool emit_indirect_pic_fast_hits(rv64_jit_writer_t *w, uint8_t *hit_disps[RV64_JIT_INDIRECT_PIC_WAYS], uint32_t completed_count,
                                        uint64_t *subset_taken_counter, rv64_jit_indirect_pic_kind_t pic_kind, uint8_t **budget_to_miss_disp)
{
    const uint32_t body_entry_off = (uint32_t)offsetof(rv64_jit_block_t, body_entry);
    uint8_t *budget_disp = NULL;

    Assert(w->indirect_pic != NULL && w->indirect_pic->state.used, "jit: missing active RV64 indirect PIC builder");
    Assert(hit_disps != NULL && hit_disps[0] != NULL && hit_disps[1] != NULL, "jit: missing RV64 indirect PIC hit fixups");
    Assert(budget_to_miss_disp != NULL, "jit: missing RV64 indirect PIC budget fixup result");
    *budget_to_miss_disp = NULL;
    ...
    if (!emit_mov_esi_imm32(w, 1) || !emit_jmp_rel32_placeholder(w, &secondary_to_common_disp))
    {
        return false;
    }
    ...
    if (!emit_indirect_target_budget(w, completed_count, &budget_disp) || !emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_pic_hits[pic_kind]))
    {
        return false;
    }

#if RV64_JIT_STATS
    if (!emit_test_esi_esi(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &no_secondary_stat_disp) ||
        !emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_pic_secondary_hits[pic_kind]))
    {
        return false;
    }
    ...
    return emit_inc_jit_stat_counter(w, &rv64_jit_stats.indirect_pic_budget_rejections[pic_kind]) &&
           emit_jmp_rel32_placeholder(w, budget_to_miss_disp);
}
 ...
 */
static bool emit_indirect_miss_path(rv64_jit_writer_t *w, const rv64_jit_link_guard_fixups_t *guard_fixups, uint8_t *pic_budget_to_miss_disp,
                                    uint8_t *jump_cache_budget_to_miss_disp, uint32_t completed_count, uint64_t *subset_miss_counter)
{
    const uint8_t *miss_path = w->cur;

    Assert(guard_fixups != NULL, "jit: missing RV64 indirect guard fixups");
    ...
    return emit_mov_rax_r9(w) && emit_store_rax_pc(w) && emit_inc_jit_stat_counter(w, &rv64_jit_stats.direct_link_miss_count) &&
           (subset_miss_counter == NULL || emit_inc_jit_stat_counter(w, subset_miss_counter)) && emit_return_total_retired(w, completed_count);
}
 ...
 */
static bool emit_indirect_pic_patch_thunks(rv64_jit_writer_t *w, uint32_t completed_count, rv64_jit_indirect_pic_kind_t pic_kind)
{
    Assert(w->indirect_pic != NULL && w->indirect_pic->state.used, "jit: missing active RV64 indirect PIC builder");
...
#if RV64_JIT_STATS
            || !emit_mov_esi_imm32(w, 1u + (uint32_t)pic_kind * RV64_JIT_INDIRECT_PIC_WAYS + i)
#endif
            || !emit_jmp_rel32_placeholder(w, &w->indirect_pic->target_disps[i]))
        {
            return false;
        }
    }
 ...
 */
static bool emit_indirect_link_exit(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t rd, vaddr_t link_pc, uint32_t completed_count,
                                    bool source_uses_data_state, bool pic_eligible, uint64_t *subset_taken_counter, uint64_t *subset_miss_counter,
                                    rv64_jit_indirect_pic_kind_t pic_kind)
{
    ...
    Assert(pic_kind < RV64_JIT_INDIRECT_PIC_KIND_COUNT, "jit: invalid RV64 indirect PIC kind %u", pic_kind);
    Assert(!(pic_enabled && jump_cache_enabled), "jit: one RV64 JALR selected both indirect caches");

    if (pic_enabled)
    {
        Assert(!w->indirect_pic->state.used, "jit: more than one indirect PIC in an RV64 block");
    ...
    if (jump_cache_enabled)
    {
        Assert(!w->indirect_jump_cache->state.used, "jit: more than one indirect jump cache in an RV64 block");
        w->indirect_jump_cache->state.used = true;
        JIT_STAT_INC(emitted_sites.indirect_jump_cache_sites);
    }

    if (!emit_mov_r9_rax(w) || !jit_reg_write_imm(w, regs, rd, link_pc) || !jit_reg_flush_all_dirty(w, regs))
    {
        return false;
    }
    ...
    if (jump_cache_enabled)
    {
        if (!emit_indirect_jump_cache_probe(w, completed_count, subset_taken_counter, &jump_cache_budget_to_miss_disp))
        {
            return false;
        }
    }
    ...
    if (!emit_link_target_guards(w, &candidate, &guard_config, &guard_fixups))
    {
        return false;
    }
    ...
    if (!emit_indirect_authoritative_hit(w, pic_enabled, jump_cache_enabled, subset_taken_counter))
    {
        return false;
    }
    ...
    if (pic_enabled)
    {
        if (!emit_indirect_pic_fast_hits(w, pic_hit_disps, completed_count, subset_taken_counter, pic_kind, &pic_budget_to_miss_disp))
        {
            return false;
        }
    }

    if (!emit_indirect_miss_path(w, &guard_fixups, pic_budget_to_miss_disp, jump_cache_budget_to_miss_disp, completed_count, subset_miss_counter))
    {
        return false;
    }

    if (pic_enabled && !emit_indirect_pic_patch_thunks(w, completed_count, pic_kind))
    {
        return false;
    }
...
static bool decode_load_descriptor(uint32_t instr, rv64_jit_load_descriptor_t *desc)
{
...
static bool decode_store_descriptor(uint32_t instr, rv64_jit_store_descriptor_t *desc)
{
...
static bool emit_direct_pmem_load_rax(rv64_jit_writer_t *w, uint32_t funct3)
{
    ...
    case RV64_FUNCT3_LB: /* LB: movsx rax, byte ptr [r10 + rdx]. */
        return emit_u8(w, 0x49) && emit_u8(w, 0x0f) && emit_u8(w, 0xbe) && emit_pmem_sib_r10_rdx(w, HOST_REG_RAX);
    case RV64_FUNCT3_LH: /* LH: movsx rax, word ptr [r10 + rdx]. */
        return emit_u8(w, 0x49) && emit_u8(w, 0x0f) && emit_u8(w, 0xbf) && emit_pmem_sib_r10_rdx(w, HOST_REG_RAX);
    case RV64_FUNCT3_LW: /* LW: movsxd rax, dword ptr [r10 + rdx]. */
        return emit_u8(w, 0x49) && emit_u8(w, 0x63) && emit_pmem_sib_r10_rdx(w, HOST_REG_RAX);
    case RV64_FUNCT3_LD: /* LD: mov rax, qword ptr [r10 + rdx]. */
        return emit_u8(w, 0x49) && emit_u8(w, 0x8b) && emit_pmem_sib_r10_rdx(w, HOST_REG_RAX);
    case RV64_FUNCT3_LBU: /* LBU: movzx eax, byte ptr [r10 + rdx]. */
        return emit_u8(w, 0x41) && emit_u8(w, 0x0f) && emit_u8(w, 0xb6) && emit_pmem_sib_r10_rdx(w, HOST_REG_RAX);
    case RV64_FUNCT3_LHU: /* LHU: movzx eax, word ptr [r10 + rdx]. */
        return emit_u8(w, 0x41) && emit_u8(w, 0x0f) && emit_u8(w, 0xb7) && emit_pmem_sib_r10_rdx(w, HOST_REG_RAX);
    case RV64_FUNCT3_LWU: /* LWU: mov eax, dword ptr [r10 + rdx]. */
        return emit_u8(w, 0x41) && emit_u8(w, 0x8b) && emit_pmem_sib_r10_rdx(w, HOST_REG_RAX);
    default:
        return false;
    }
}
...
static bool emit_direct_mmio_load_rax(rv64_jit_writer_t *w, uint32_t funct3)
{
    ...
    case RV64_FUNCT3_LB: /* LB: movsx rax, byte ptr [rcx + rdx]. */
        return emit_u8(w, 0x48) && emit_u8(w, 0x0f) && emit_u8(w, 0xbe) && emit_mmio_sib_rcx_rdx(w, HOST_REG_RAX);
    case RV64_FUNCT3_LH: /* LH: movsx rax, word ptr [rcx + rdx]. */
        return emit_u8(w, 0x48) && emit_u8(w, 0x0f) && emit_u8(w, 0xbf) && emit_mmio_sib_rcx_rdx(w, HOST_REG_RAX);
    case RV64_FUNCT3_LW: /* LW: movsxd rax, dword ptr [rcx + rdx]. */
        return emit_u8(w, 0x48) && emit_u8(w, 0x63) && emit_mmio_sib_rcx_rdx(w, HOST_REG_RAX);
    case RV64_FUNCT3_LD: /* LD: mov rax, qword ptr [rcx + rdx]. */
        return emit_u8(w, 0x48) && emit_u8(w, 0x8b) && emit_mmio_sib_rcx_rdx(w, HOST_REG_RAX);
    case RV64_FUNCT3_LBU: /* LBU: movzx eax, byte ptr [rcx + rdx]. */
        return emit_u8(w, 0x0f) && emit_u8(w, 0xb6) && emit_mmio_sib_rcx_rdx(w, HOST_REG_RAX);
    case RV64_FUNCT3_LHU: /* LHU: movzx eax, word ptr [rcx + rdx]. */
        return emit_u8(w, 0x0f) && emit_u8(w, 0xb7) && emit_mmio_sib_rcx_rdx(w, HOST_REG_RAX);
    case RV64_FUNCT3_LWU: /* LWU: mov eax, dword ptr [rcx + rdx]. */
        return emit_u8(w, 0x8b) && emit_mmio_sib_rcx_rdx(w, HOST_REG_RAX);
    default:
        return false;
    }
}
...
static bool emit_direct_mmio_load_rax_from_rdx(rv64_jit_writer_t *w, uint32_t funct3)
{
    switch (funct3)
    {
    case RV64_FUNCT3_LB: /* movsx rax, byte ptr [rdx]. */
        return emit_u8(w, 0x48) && emit_u8(w, 0x0f) && emit_u8(w, 0xbe) && emit_u8(w, 0x02);
    case RV64_FUNCT3_LH: /* movsx rax, word ptr [rdx]. */
        return emit_u8(w, 0x48) && emit_u8(w, 0x0f) && emit_u8(w, 0xbf) && emit_u8(w, 0x02);
    case RV64_FUNCT3_LW: /* movsxd rax, dword ptr [rdx]. */
        return emit_u8(w, 0x48) && emit_u8(w, 0x63) && emit_u8(w, 0x02);
    case RV64_FUNCT3_LD: /* mov rax, qword ptr [rdx]. */
        return emit_u8(w, 0x48) && emit_u8(w, 0x8b) && emit_u8(w, 0x02);
    case RV64_FUNCT3_LBU: /* movzx eax, byte ptr [rdx]. */
        return emit_u8(w, 0x0f) && emit_u8(w, 0xb6) && emit_u8(w, 0x02);
    case RV64_FUNCT3_LHU: /* movzx eax, word ptr [rdx]. */
        return emit_u8(w, 0x0f) && emit_u8(w, 0xb7) && emit_u8(w, 0x02);
 ...
 */
static bool direct_mmio_load_address_observed(uint64_t addr, uint32_t len, uint64_t *host_ptr)
{
    ...
    for (size_t i = 0; i < direct_map_count; i++)
    {
        ...
        const uint64_t map_size = (uint64_t)map->high - (uint64_t)map->low + 1u;

        if (map_size >= len && addr >= (uint64_t)map->low && addr - (uint64_t)map->low <= map_size - len)
        {
            *host_ptr = (uint64_t)(uintptr_t)map->space + (addr - (uint64_t)map->low);
            return true;
        }
    }
#else
 ...
 */
static bool emit_direct_mmio_load_route_probe(rv64_jit_writer_t *w, rv64_jit_mmio_route_builder_t *routes, uint8_t site, uint32_t funct3,
                                              uint8_t **success_disp)
{
    uint8_t *tag_miss_disp = NULL;

    if (!emit_mmio_route_cmp_tag_rax(w, routes, site) || !emit_jcc_rel32_placeholder(w, HOST_JCC_NE, &tag_miss_disp) ||
        !emit_mmio_route_load_host_rdx(w, routes, site) || !emit_direct_mmio_load_rax_from_rdx(w, funct3) ||
        !emit_inc_jit_stat_counter_preserve_rax(w, &rv64_jit_stats.direct_mmio_load_route_hits) ||
        !emit_inc_jit_stat_counter_preserve_rax(w, &rv64_jit_stats.inline_direct_mmio_load_hits) || !emit_jmp_rel32_placeholder(w, success_disp))
    {
        return false;
    }
    ...
    return emit_inc_jit_stat_counter_preserve_rax(w, &rv64_jit_stats.direct_mmio_load_route_misses);
}
 ...
 */
static bool emit_inline_direct_mmio_loads(rv64_jit_writer_t *w, uint32_t funct3, uint32_t len, rv64_jit_mmio_route_builder_t *routes,
                                          uint8_t route_site, uint8_t *success_disps[RV64_JIT_DIRECT_MMIO_MAX_MAPS], uint32_t *success_count)
{
    *success_count = 0;
    ...
    for (size_t i = 0; i < direct_map_count; i++)
    {
        ...
        const uint64_t map_size = (uint64_t)map->high - (uint64_t)map->low + 1u;
        Assert(map_size >= len, "jit: direct MMIO map %s is smaller than width %u", map->name, len);
         ...
         */
        if (!emit_mov_rdx_rax(w) || !emit_movabs_rcx(w, (uint64_t)map->low) || !emit_sub_rdx_rcx(w) || !emit_movabs_rcx(w, map_size - len) ||
            !emit_cmp_rdx_rcx(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_A, &next_map_disp))
        {
            return false;
        }
        ...
        if (route_site != RV64_JIT_MMIO_ROUTE_NO_SITE)
        {
             ...
             */
            if (!emit_movabs_rcx(w, (uint64_t)(uintptr_t)map->space) || !emit_add_rdx_rcx(w) || !emit_mov_rdi_rax(w) ||
                !emit_direct_mmio_load_rax_from_rdx(w, funct3) || !emit_mmio_route_store_host_rdx(w, routes, route_site) ||
                !emit_mmio_route_store_tag_rdi(w, routes, route_site) ||
                !emit_inc_jit_stat_counter_preserve_rax(w, &rv64_jit_stats.direct_mmio_load_route_fills) ||
                !emit_inc_jit_stat_counter_preserve_rax(w, &rv64_jit_stats.inline_direct_mmio_load_hits) ||
                !emit_jmp_rel32_placeholder(w, &success_disps[*success_count]))
            {
                return false;
            }
        }
             ...
             */
            !emit_inc_jit_stat_counter(w, &rv64_jit_stats.inline_direct_mmio_load_hits) || !emit_movabs_rcx(w, (uint64_t)(uintptr_t)map->space) ||
            !emit_direct_mmio_load_rax(w, funct3) || !emit_jmp_rel32_placeholder(w, &success_disps[*success_count]))
        {
            return false;
        }
...
static bool emit_tlb_guard_slow_jcc(rv64_jit_writer_t *w, rv64_jit_tlb_guard_patch_t *patch, uint8_t jcc_opcode)
{
    Assert(patch->count < sizeof(patch->slow_disps) / sizeof(patch->slow_disps[0]), "jit: too many RV64 DTLB slow-path branches");
...
static bool emit_inline_dtlb_lookup_to_pmem_offset(rv64_jit_writer_t *w, uint32_t len, uint32_t need_access, rv64_jit_tlb_guard_patch_t *patch)
{
    ...
    const uint32_t pg_paddr_off = (uint32_t)offsetof(rv64_jit_data_tlb_entry_t, pg_paddr);
     ...
     */
    if (!emit_mov_rdx_rax(w) || !emit_shr_rdx_imm(w, PAGE_SHIFT) || !emit_mov_r8_rdx(w) || !emit_shr_r8_imm(w, RV64_JIT_DATA_TLB_VPN_MIX_SHIFT) ||
        !emit_xor_r8_rdx(w) || !emit_movabs_rdx(w, satp ^ (satp >> RV64_JIT_DATA_TLB_SATP_MIX_SHIFT) ^ state) || !emit_xor_r8_rdx(w) ||
        !emit_and_r8d_imm(w, RV64_JIT_DATA_TLB_SIZE - 1u) || !emit_shl_r8_imm(w, entry_shift) ||
        !emit_movabs_rdx(w, (uint64_t)(uintptr_t)rv64_jit_data_tlb) || !emit_add_r8_rdx(w) || !emit_cmp_r8b_field_imm8(w, valid_off, 0) ||
        !emit_tlb_guard_slow_jcc(w, patch, HOST_JCC_E) || !emit_movabs_rdx(w, satp) || !emit_cmp_r8q_field_rdx(w, satp_off) ||
        !emit_tlb_guard_slow_jcc(w, patch, HOST_JCC_NE) || !emit_mov_rdx_rax(w) || !emit_shr_rdx_imm(w, PAGE_SHIFT) ||
        !emit_cmp_r8q_field_rdx(w, vpn_off) || !emit_tlb_guard_slow_jcc(w, patch, HOST_JCC_NE) || !emit_cmp_r8d_field_imm32(w, state_off, state) ||
        !emit_tlb_guard_slow_jcc(w, patch, HOST_JCC_NE) || !emit_test_r8d_field_imm32(w, access_off, need_access) ||
        !emit_tlb_guard_slow_jcc(w, patch, HOST_JCC_E) || !emit_and_rax_imm32(w, PAGE_MASK) || !emit_cmp_rax_imm32(w, PAGE_SIZE - len) ||
        !emit_tlb_guard_slow_jcc(w, patch, HOST_JCC_A) || !emit_mov_rdx_r8q_field(w, pg_paddr_off) || !emit_or_rdx_rax(w) ||
        !emit_movabs_rax(w, (uint64_t)CONFIG_MBASE) || !emit_sub_rdx_rax(w))
    {
        return false;
    }
...
static bool emit_inline_sv39_load_fast_path(rv64_jit_writer_t *w, uint32_t funct3, uint32_t len, rv64_jit_tlb_guard_patch_t *patch)
{
     ...
     */
    return emit_inline_dtlb_lookup_to_pmem_offset(w, len, RV64_JIT_DATA_TLB_READ, patch) && emit_inline_paged_load_hit_stats(w) &&
...
static bool emit_direct_pmem_store_from_rcx(rv64_jit_writer_t *w, uint32_t len)
{
    ...
    case 1: /* mov byte ptr [r10 + rdx], cl. */
        return emit_u8(w, 0x41) && emit_u8(w, 0x88) && emit_pmem_sib_r10_rdx(w, HOST_REG_RCX);
    case 2: /* mov word ptr [r10 + rdx], cx. */
        return emit_u8(w, 0x66) && emit_u8(w, 0x41) && emit_u8(w, 0x89) && emit_pmem_sib_r10_rdx(w, HOST_REG_RCX);
    case 4: /* mov dword ptr [r10 + rdx], ecx. */
        return emit_u8(w, 0x41) && emit_u8(w, 0x89) && emit_pmem_sib_r10_rdx(w, HOST_REG_RCX);
    case 8: /* mov qword ptr [r10 + rdx], rcx. */
        return emit_u8(w, 0x49) && emit_u8(w, 0x89) && emit_pmem_sib_r10_rdx(w, HOST_REG_RCX);
    default:
        return false;
    }
}
...
static bool emit_direct_mmio_store_from_rcx(rv64_jit_writer_t *w, uint32_t len)
{
    /*
    ...
    case 1: /* mov byte ptr [rdi + rdx], cl. */
        return emit_u8(w, 0x88) && emit_mmio_sib_rdi_rdx(w, HOST_REG_RCX);
    case 2: /* mov word ptr [rdi + rdx], cx. */
        return emit_u8(w, 0x66) && emit_u8(w, 0x89) && emit_mmio_sib_rdi_rdx(w, HOST_REG_RCX);
    case 4: /* mov dword ptr [rdi + rdx], ecx. */
        return emit_u8(w, 0x89) && emit_mmio_sib_rdi_rdx(w, HOST_REG_RCX);
    case 8: /* mov qword ptr [rdi + rdx], rcx. */
        return emit_u8(w, 0x48) && emit_u8(w, 0x89) && emit_mmio_sib_rdi_rdx(w, HOST_REG_RCX);
    default:
        return false;
    }
}
...
static bool emit_direct_mmio_store_from_rcx_to_rdi(rv64_jit_writer_t *w, uint32_t len)
{
    switch (len)
    {
    ...
    case 2: /* mov word ptr [rdi], cx. */
        return emit_u8(w, 0x66) && emit_u8(w, 0x89) && emit_u8(w, 0x0f);
    case 4: /* mov dword ptr [rdi], ecx. */
        return emit_u8(w, 0x89) && emit_u8(w, 0x0f);
    case 8: /* mov qword ptr [rdi], rcx. */
        return emit_u8(w, 0x48) && emit_u8(w, 0x89) && emit_u8(w, 0x0f);
    default:
        return false;
    }
}
 ...
 */
static bool direct_mmio_store_address_observed(uint64_t addr, uint32_t len, uint64_t *host_ptr)
{
    ...
    {
        const IODirectWriteRegion *region = mmio_direct_write_region(i);
        ...
        const uint64_t region_size = (uint64_t)region->high - (uint64_t)region->low + 1u;

        if (region_size >= len && addr >= (uint64_t)region->low && addr - (uint64_t)region->low <= region_size - len)
        {
            *host_ptr = (uint64_t)(uintptr_t)region->space + (addr - (uint64_t)region->low);
            return true;
        }
    }
#else
...
static bool emit_direct_mmio_store_route_probe(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, rv64_jit_mmio_route_builder_t *routes, uint8_t site,
                                               uint32_t rs2, uint32_t len, uint8_t **success_disp)
{
    uint8_t *tag_miss_disp = NULL;

    if (!emit_mmio_route_cmp_tag_rax(w, routes, site) || !emit_jcc_rel32_placeholder(w, HOST_JCC_NE, &tag_miss_disp) ||
        !emit_mmio_route_load_host_rdi(w, routes, site) || !jit_reg_read_rcx(w, regs, rs2) || !emit_direct_mmio_store_from_rcx_to_rdi(w, len) ||
        !emit_inc_jit_stat_counter(w, &rv64_jit_stats.direct_mmio_store_route_hits) ||
        !emit_inc_jit_stat_counter(w, &rv64_jit_stats.inline_direct_mmio_store_hits) || !emit_jmp_rel32_placeholder(w, success_disp))
    {
        return false;
    }
    ...
    return emit_inc_jit_stat_counter_preserve_rax(w, &rv64_jit_stats.direct_mmio_store_route_misses);
}
 ...
 */
static bool emit_inline_direct_mmio_stores(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t rs2, uint32_t len,
                                           rv64_jit_mmio_route_builder_t *routes, uint8_t route_site,
                                           uint8_t *success_disps[RV64_JIT_DIRECT_MMIO_MAX_REGIONS], uint32_t *success_count)
{
    *success_count = 0;
    ...
    for (size_t i = 0; i < direct_region_count; i++)
    {
        ...
        Assert(region != NULL, "jit: direct MMIO write region index %zu disappeared", i);
        ...
        const uint64_t region_size = (uint64_t)region->high - (uint64_t)region->low + 1u;
        Assert(region_size >= len, "jit: direct MMIO write region %s is smaller than width %u", region->map_name, len);
         ...
         */
        if (!emit_mov_rdx_rax(w) || !emit_movabs_rcx(w, (uint64_t)region->low) || !emit_sub_rdx_rcx(w) || !emit_movabs_rcx(w, region_size - len) ||
            !emit_cmp_rdx_rcx(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_A, &next_region_disp))
        {
            return false;
        }
        ...
        if (route_site != RV64_JIT_MMIO_ROUTE_NO_SITE)
        {
            if (!jit_reg_read_rcx(w, regs, rs2) || !emit_movabs_rdi(w, (uint64_t)(uintptr_t)region->space) || !emit_add_rdi_rdx(w) ||
                !emit_direct_mmio_store_from_rcx_to_rdi(w, len) || !emit_mmio_route_store_host_rdi(w, routes, route_site) ||
                !emit_mmio_route_store_tag_rax(w, routes, route_site) ||
                !emit_inc_jit_stat_counter(w, &rv64_jit_stats.direct_mmio_store_route_fills) ||
                !emit_inc_jit_stat_counter(w, &rv64_jit_stats.inline_direct_mmio_store_hits) ||
                !emit_jmp_rel32_placeholder(w, &success_disps[*success_count]))
            {
                return false;
            }
        }
        else if (!jit_reg_read_rcx(w, regs, rs2) || !emit_movabs_rdi(w, (uint64_t)(uintptr_t)region->space) ||
                 !emit_direct_mmio_store_from_rcx(w, len) || !emit_inc_jit_stat_counter(w, &rv64_jit_stats.inline_direct_mmio_store_hits) ||
                 !emit_jmp_rel32_placeholder(w, &success_disps[*success_count]))
        {
            return false;
        }
...
static bool emit_guard_store_not_compiled_source(rv64_jit_writer_t *w, uint32_t len, uint8_t **cross_chunk_disp, uint8_t **source_chunk_disp)
{
     ...
     */
    return emit_mov_r8d_edx(w) && emit_and_r8d_imm(w, RV64_JIT_SOURCE_CHUNK_MASK) && emit_cmp_r8d_imm(w, RV64_JIT_SOURCE_CHUNK_SIZE - len) &&
           emit_jcc_rel32_placeholder(w, HOST_JCC_A, cross_chunk_disp) && emit_mov_r8d_edx(w) && emit_shr_r8d_imm(w, RV64_JIT_SOURCE_CHUNK_SHIFT) &&
           emit_movabs_rax(w, (uint64_t)(uintptr_t)rv64_jit_source_chunk_refs) && emit_cmp_u32_ref_zero_rax_r8(w) &&
...
static bool emit_guard_store_not_translation_dependency(rv64_jit_writer_t *w, uint8_t **data_page_table_disp, uint8_t **ifetch_page_table_disp)
{
     ...
     */
    return emit_mov_r8d_edx(w) && emit_shr_r8d_imm(w, PAGE_SHIFT) && emit_movabs_rax(w, (uint64_t)(uintptr_t)rv64_jit_data_tlb_pt_page_refs) &&
           emit_cmp_u16_ref_zero_rax_r8(w) && emit_jcc_rel32_placeholder(w, HOST_JCC_NE, data_page_table_disp) &&
           emit_movabs_rax(w, (uint64_t)(uintptr_t)rv64_jit_ifetch_pt_page_refs) && emit_cmp_u32_ref_zero_rax_r8(w) &&
...
static bool emit_inline_sv39_store_address(rv64_jit_writer_t *w, uint32_t len, rv64_jit_tlb_guard_patch_t *patch)
{
     ...
     */
    return emit_inline_dtlb_lookup_to_pmem_offset(w, len, RV64_JIT_DATA_TLB_WRITE, patch);
}
...
static bool emit_alignment_guard_al(rv64_jit_writer_t *w, uint32_t len, uint8_t **slow_disp)
{
    ...
    return emit_test_al_imm8(w, (uint8_t)(len - 1u)) && emit_jcc_rel32_placeholder(w, HOST_JCC_NE, slow_disp);
}
...
static bool emit_guard_bare_address_in_pmem(rv64_jit_writer_t *w, uint32_t len, uint8_t **slow_disp)
{
     ...
     */
    return emit_movabs_rcx(w, (uint64_t)CONFIG_MBASE) && emit_sub_rdx_rcx(w) && emit_movabs_rcx(w, (uint64_t)CONFIG_MSIZE - len) &&
           emit_cmp_rdx_rcx(w) && emit_jcc_rel32_placeholder(w, HOST_JCC_A, slow_disp);
}
 ...
 */
static bool emit_bare_load_value_rax(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, const rv64_jit_load_descriptor_t *desc, vaddr_t pc,
                                     rv64_jit_mmio_route_builder_t *mmio_routes, uint32_t *direct_mmio_done_count)
{
    uint8_t *range_slow_disp = NULL;
    ...
    Assert(direct_mmio_done_count != NULL, "jit: missing Bare load MMIO-site output");

    observed_addr = observed_bare_address(desc->rs1, desc->imm);
    *direct_mmio_done_count = 0;

    if (direct_mmio_load_address_observed(observed_addr, desc->len, &observed_host))
    {
        route_site = mmio_route_reserve_site(mmio_routes, observed_addr, observed_host);
    }
     ...
     */
    if (route_site != RV64_JIT_MMIO_ROUTE_NO_SITE &&
        !emit_direct_mmio_load_route_probe(w, mmio_routes, route_site, desc->funct3, &cached_mmio_done_disp))
    {
        return false;
    }
    ...
    if (!emit_inline_direct_mmio_loads(w, desc->funct3, desc->len, mmio_routes, route_site, direct_mmio_done_disps, direct_mmio_done_count))
    {
        return false;
    }
                                  ...
                                  uint32_t completed_count)
{
    ...
    if (!jit_reg_read_rax(w, regs, desc->rs1) || !emit_add_rax_imm32(w, desc->imm))
    {
        return false;
    }
     ...
     */
    if (!emit_mov_rcx_rax(w) || !emit_inline_sv39_load_fast_path(w, desc->funct3, desc->len, &tlb_guard))
    {
        return false;
    }
...
static bool emit_bare_load_instr(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, const rv64_jit_load_descriptor_t *desc, vaddr_t pc,
...
bool rv64_jit_emit_load_instr(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr, vaddr_t pc, uint32_t completed_count,
                              rv64_jit_mmio_route_builder_t *mmio_routes)
{
...
static bool emit_paged_store_instr(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, const rv64_jit_store_descriptor_t *desc, vaddr_t pc,
 ...
 */
static bool emit_bare_store_pmem_region(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, const rv64_jit_store_descriptor_t *desc, vaddr_t pc,
                                        vaddr_t next_pc, uint32_t completed_count, rv64_jit_bare_store_continuation_fixups_t *continuations)
{
    uint8_t *cross_chunk_disp = NULL;
    uint8_t *source_chunk_disp = NULL;
    uint8_t *data_page_table_disp = NULL;
 ...
 */
static bool emit_bare_store_mmio_region(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, const rv64_jit_store_descriptor_t *desc, vaddr_t pc,
                                        vaddr_t next_pc, uint32_t completed_count, rv64_jit_mmio_route_builder_t *mmio_routes, uint8_t route_site,
                                        rv64_jit_bare_store_continuation_fixups_t *continuations)
{
     ...
     */
    if (!emit_inline_direct_mmio_stores(w, regs, desc->rs2, desc->len, mmio_routes, route_site, continuations->direct_mmio_done_disps,
...
static void patch_bare_store_continuations(rv64_jit_writer_t *w, const rv64_jit_bare_store_continuation_fixups_t *continuations)
{
...
static bool emit_bare_store_instr(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, const rv64_jit_store_descriptor_t *desc, vaddr_t pc,
                                  vaddr_t next_pc, uint32_t completed_count, rv64_jit_mmio_route_builder_t *mmio_routes)
{
    uint8_t *align_slow_disp = NULL;
    uint8_t *range_slow_disp = NULL;
    ...
    uint8_t route_site = RV64_JIT_MMIO_ROUTE_NO_SITE;
    const uint64_t observed_addr = observed_bare_address(desc->rs1, desc->imm);
    uint64_t observed_host = 0;

    if (direct_mmio_store_address_observed(observed_addr, desc->len, &observed_host))
    {
        route_site = mmio_route_reserve_site(mmio_routes, observed_addr, observed_host);
    }
     ...
     */
    if (route_site != RV64_JIT_MMIO_ROUTE_NO_SITE &&
        !emit_direct_mmio_store_route_probe(w, regs, mmio_routes, route_site, desc->rs2, desc->len, &continuations.cached_mmio_done_disp))
    {
        return false;
    }
    ...
    if (!emit_bare_store_mmio_region(w, regs, desc, pc, next_pc, completed_count, mmio_routes, route_site, &continuations))
    {
        return false;
    }
...
bool rv64_jit_emit_store_instr(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr, vaddr_t pc, vaddr_t next_pc,
                               uint32_t completed_count, rv64_jit_mmio_route_builder_t *mmio_routes)
{
    ...
    return emit_paged_store_instr(w, regs, &desc, pc, next_pc, completed_count);
}
...
{
    if (!emit_inc_jit_stat_counter(w, &rv64_jit_stats.native_m_executions[op]))
    {
        return false;
    }
...
static bool emit_rv64_mul_high(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t rd, rv64_jit_m_op_t op)
{
    switch (op)
    {
    ...
    case RV64_JIT_M_OP_MULHSU:
         ...
         */
        if (!emit_mov_rdi_rax(w) || !emit_sar_rdi_63(w) || !emit_and_rdi_rcx(w) || !emit_mul_rcx(w) || !emit_sub_rdx_rdi(w))
        {
            return false;
        }
        break;
    ...
    return emit_mov_rax_rdx(w) && jit_reg_write_rax(w, regs, rd) && emit_finish_native_m(w, op);
}

/* Emit full-width DIV/DIVU/REM/REMU with RISC-V's non-trapping edge results. */
static bool emit_rv64_divrem(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t rd, bool is_signed, bool want_remainder, rv64_jit_m_op_t op)
{
    uint8_t *zero_disp = NULL;
    uint8_t *normal_done_disp = NULL;

    if (!emit_test_rcx_rcx(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &zero_disp))
    {
        return false;
    }
    ...
    if (!is_signed)
    {
        if (!emit_zero_rdx(w) || !emit_div_rcx(w) || (want_remainder && !emit_mov_rax_rdx(w)) || !emit_jmp_rel32_placeholder(w, &normal_done_disp))
        {
            return false;
        }
        ...
        return jit_reg_write_rax(w, regs, rd) && emit_finish_native_m(w, op);
    }
     ...
     */
    if (!emit_movabs_rdx(w, UINT64_C(0x8000000000000000)) || !emit_cmp_rax_rdx(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_NE, &normal_disp) ||
        !emit_cmp_rcx_neg_one(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &overflow_disp))
    {
        return false;
    }
    ...
    if (!emit_cqo(w) || !emit_idiv_rcx(w) || (want_remainder && !emit_mov_rax_rdx(w)) || !emit_jmp_rel32_placeholder(w, &normal_done_disp))
    {
        return false;
    }
    ...
    if ((!want_remainder && !emit_movabs_rax(w, UINT64_MAX)) || !emit_jmp_rel32_placeholder(w, &zero_done_disp))
    {
        return false;
    }
...
static bool emit_rv64_divrem_word(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t rd, bool is_signed, bool want_remainder,
                                  rv64_jit_m_op_t op)
{
    uint8_t *zero_disp = NULL;
    uint8_t *normal_done_disp = NULL;

    if (!emit_test_ecx_ecx(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_E, &zero_disp))
    {
        return false;
    }
    ...
    if (!is_signed)
    {
        if (!emit_zero_rdx(w) || !emit_div_ecx(w) || (want_remainder && !emit_mov_eax_edx(w)) || !emit_jmp_rel32_placeholder(w, &normal_done_disp))
        {
            return false;
        }
    ...
    else
    {
        ...
        if (!emit_cmp_eax_imm32(w, (uint32_t)INT32_MIN) || !emit_jcc_rel32_placeholder(w, HOST_JCC_NE, &normal_disp) || !emit_cmp_ecx_neg_one(w) ||
        ...
        if (!emit_cdq(w) || !emit_idiv_ecx(w) || (want_remainder && !emit_mov_eax_edx(w)) || !emit_jmp_rel32_placeholder(w, &normal_done_disp))
        {
            return false;
        }
        ...
        if ((!want_remainder && !emit_mov_eax_imm32(w, UINT32_MAX)) || !emit_jmp_rel32_placeholder(w, &zero_done_disp))
        {
            return false;
        }
    ...
    return emit_cdqe(w) && jit_reg_write_rax(w, regs, rd) && emit_finish_native_m(w, op);
}
...
static bool emit_op_imm(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr)
{
    const uint32_t rd = rv64_instr_rd(instr);
    ...
    case RV64_FUNCT3_SLT: /* SLTI, signed compare. */
        ...
        return emit_cmp_rax_imm32(w, imm) && emit_setcc_rax(w, HOST_SETCC_L) && jit_reg_write_rax(w, regs, rd);
    case RV64_FUNCT3_SLTU: /* SLTIU, unsigned compare. */
        ...
        return emit_cmp_rax_imm32(w, imm) && emit_setcc_rax(w, HOST_SETCC_B) && jit_reg_write_rax(w, regs, rd);
    ...
    case RV64_FUNCT3_SLL: /* SLLI; funct6 must be 000000 for RV64 base shifts. */
        ...
        return emit_shift_imm_hreg(w, regs, rd, rs1, HOST_SHIFT_GROUP_SAL, (uint8_t)rv64_instr_shamt6(instr));
    case RV64_FUNCT3_SRL_SRA: /* SRLI/SRAI; funct6 selects logical versus arithmetic right shift. */
        if (rv64_instr_funct6(instr) == RV64_FUNCT6_SHIFT_LOGICAL)
        {
            return emit_shift_imm_hreg(w, regs, rd, rs1, HOST_SHIFT_GROUP_SHR, (uint8_t)rv64_instr_shamt6(instr));
        }

        if (rv64_instr_funct6(instr) == RV64_FUNCT6_SHIFT_ARITH)
        {
            return emit_shift_imm_hreg(w, regs, rd, rs1, HOST_SHIFT_GROUP_SAR, (uint8_t)rv64_instr_shamt6(instr));
        }
...
static bool emit_op_imm32(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr)
{
    const uint32_t rd = rv64_instr_rd(instr);
    const uint32_t funct3 = rv64_instr_funct3(instr);
    const uint32_t rs1 = rv64_instr_rs1(instr);
    ...
    case RV64_FUNCT3_SLL: /* SLLIW; funct7 must be zero and shamt is five bits. */
        ...
        return jit_reg_read_rax(w, regs, rs1) && emit_shift_eax_imm_sext(w, HOST_SHIFT_SAL, (uint8_t)rv64_instr_shamt5(instr)) &&
               jit_reg_write_rax(w, regs, rd);
    case RV64_FUNCT3_SRL_SRA: /* SRLIW/SRAIW; funct7 distinguishes logical from arithmetic. */
        if (rv64_instr_funct7(instr) == RV64_FUNCT7_BASE)
        {
            return jit_reg_read_rax(w, regs, rs1) && emit_shift_eax_imm_sext(w, HOST_SHIFT_SHR, (uint8_t)rv64_instr_shamt5(instr)) &&
                   jit_reg_write_rax(w, regs, rd);
        }

        if (rv64_instr_funct7(instr) == RV64_FUNCT7_SUB_SRA)
        {
            return jit_reg_read_rax(w, regs, rs1) && emit_shift_eax_imm_sext(w, HOST_SHIFT_SAR, (uint8_t)rv64_instr_shamt5(instr)) &&
                   jit_reg_write_rax(w, regs, rd);
        }
...
static bool emit_rv64m_op(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr)
{
    const uint32_t rd = rv64_instr_rd(instr);
    const uint32_t funct3 = rv64_instr_funct3(instr);
    const uint32_t rs1 = rv64_instr_rs1(instr);
    ...
    if (rs2 == RV64_GPR_ZERO)
    {
        ...
        {
            return jit_reg_write_imm(w, regs, rd, UINT64_MAX) && emit_finish_native_m(w, op);
        }

        if (op == RV64_JIT_M_OP_REM || op == RV64_JIT_M_OP_REMU)
        {
            return jit_reg_copy(w, regs, rd, rs1) && emit_finish_native_m(w, op);
        }
    }
    ...
    case RV64_JIT_M_OP_MUL:
        return emit_u8(w, 0x48) && emit_u8(w, 0x0f) && emit_u8(w, 0xaf) && emit_u8(w, 0xc1) && jit_reg_write_rax(w, regs, rd) &&
...
static bool emit_rv64m_op32(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr)
{
    const uint32_t rd = rv64_instr_rd(instr);
    const uint32_t funct3 = rv64_instr_funct3(instr);
    const uint32_t rs1 = rv64_instr_rs1(instr);
    ...
    if (rs2 == RV64_GPR_ZERO)
    {
         ...
         */
        return jit_reg_read_rax(w, regs, rs1) && emit_cdqe(w) && jit_reg_write_rax(w, regs, rd) && emit_finish_native_m(w, op);
    }

    if (op == RV64_JIT_M_OP_MULW)
    {
        return emit_op32_hreg_commutative(w, regs, rd, rs1, rs2, HOST_GROUP1_ADD, true) && emit_finish_native_m(w, op);
    }
...
static bool emit_op(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr)
{
    const uint32_t rd = rv64_instr_rd(instr);
    const uint32_t funct3 = rv64_instr_funct3(instr);
    const uint32_t rs1 = rv64_instr_rs1(instr);
    ...
    if (key == RV64_OP_KEY(RV64_FUNCT7_BASE, RV64_FUNCT3_SLT) || key == RV64_OP_KEY(RV64_FUNCT7_BASE, RV64_FUNCT3_SLTU))
    {
        const uint8_t setcc_opcode = key == RV64_OP_KEY(RV64_FUNCT7_BASE, RV64_FUNCT3_SLT) ? HOST_SETCC_L : HOST_SETCC_B;
        bool handled = false;

        if (!jit_reg_try_emit_cached_compare(w, regs, rd, rs1, rs2, setcc_opcode, &handled))
        {
            return false;
        }
    ...
    case RV64_OP_KEY(RV64_FUNCT7_BASE, RV64_FUNCT3_SLT): /* SLT, signed compare. */
        return emit_cmp_rax_rcx(w) && emit_setcc_rax(w, HOST_SETCC_L) && jit_reg_write_rax(w, regs, rd);
    case RV64_OP_KEY(RV64_FUNCT7_BASE, RV64_FUNCT3_SLTU): /* SLTU, unsigned compare. */
        return emit_cmp_rax_rcx(w) && emit_setcc_rax(w, HOST_SETCC_B) && jit_reg_write_rax(w, regs, rd);
    case RV64_OP_KEY(RV64_FUNCT7_BASE, RV64_FUNCT3_XOR): /* XOR */
...
static bool emit_op32(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr)
{
    const uint32_t rd = rv64_instr_rd(instr);
    const uint32_t funct3 = rv64_instr_funct3(instr);
    const uint32_t rs1 = rv64_instr_rs1(instr);
...
static bool emit_branch_chain_backedge(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, vaddr_t target, uint32_t exit_count,
                                       const uint8_t *target_native, bool stable_register_mapping)
{
    uint8_t *over_budget_disp = NULL;
    ...
    {
        if (!emit_add_r9d_imm32(w, exit_count) || !emit_sub_esi_imm32(w, exit_count) || !emit_jcc_rel32_placeholder(w, HOST_JCC_AE, &loop_disp))
        {
            return false;
        }
    ...
    {
        if (!emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv64_jit_loop_extra) || !emit_mov_eax_m32_rdx(w) || !emit_add_eax_imm32(w, exit_count) ||
            !emit_mov_ecx_eax(w) || !emit_add_ecx_imm32(w, exit_count) || !emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv64_jit_entry_budget) ||
            !emit_cmp_ecx_m32_rdx(w) || !emit_jcc_rel32_placeholder(w, HOST_JCC_A, &over_budget_disp) || !jit_reg_emit_flush_all_dirty(w, regs) ||
            !emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv64_jit_loop_extra) || !emit_mov_m32_rdx_eax(w) || !emit_jmp_rel32_placeholder(w, &loop_disp))
        {
            return false;
        }
     ...
     */
    return emit_mov_ecx_eax(w) && jit_reg_emit_flush_all_dirty(w, regs) && emit_store_pc_imm(w, target) &&
           emit_inc_jit_stat_counter(w, &rv64_jit_stats.side_exit_by_reason[RV64_JIT_SIDE_EXIT_CHAINED_OVER_BUDGET]) && emit_mov_eax_ecx(w) &&
           emit_return_eax(w);
}

/* Emit one conditional branch with a taken side exit and fall-through fast path. */
bool rv64_jit_emit_branch(rv64_jit_writer_t *w, rv64_jit_reg_cache_t *regs, uint32_t instr, vaddr_t pc, vaddr_t block_start_pc,
                          const uint8_t *native_body_entry, bool can_chain_self_backedge, bool stable_self_backedge, bool *emitted_native_backedge,
                          uint32_t retired_including_current, bool current_block_uses_data_translation_state)
{
    const uint32_t funct3 = rv64_instr_funct3(instr);
    const uint32_t rs1 = rv64_instr_rs1(instr);
    const uint32_t rs2 = rv64_instr_rs2(instr);
    ...
    else if (!jit_reg_read_rax(w, regs, rs1) || !jit_reg_read_rcx(w, regs, rs2) || !emit_cmp_rax_rcx(w))
    {
        return false;
    }

    if (!emit_jcc_rel32_placeholder(w, inverse_jcc, &fallthrough_disp))
    {
        return false;
    }
    ...
    {
        if (!emit_branch_chain_backedge(w, regs, target, retired_including_current, native_body_entry, stable_self_backedge))
        {
            return false;
        }
    ...
    {
        if (!rv64_jit_emit_plain_block_exit(w, regs, target, retired_including_current))
        {
            return false;
        }
    }
    else if (!rv64_jit_emit_direct_link_exit(w, regs, target, retired_including_current, current_block_uses_data_translation_state,
     ...
     */
    if (stable_self_backedge && can_chain_self_backedge && target == block_start_pc)
    {
        return emit_movabs_rdx(w, (uint64_t)(uintptr_t)&rv64_jit_loop_extra) && emit_mov_m32_rdx_r9d(w);
    }
                              ...
                              bool source_uses_data_state)
{
    ...
    if (opcode == RV64_OPCODE_JAL)
    {
        ...
        const bool emitted =
            emit_movabs_rax(w, link) && jit_reg_write_rax(w, regs, rd) &&
            (rv64_jit_direct_link_enabled() ? rv64_jit_emit_direct_link_exit(w, regs, target, completed_count + 1u, source_uses_data_state, NULL)
    ...
    if (can_link_indirect)
    {
        uint64_t *subset_taken_counter =
            canonical_return ? &rv64_jit_stats.direct_return_link_taken_count : &rv64_jit_stats.direct_jalr_link_taken_count;
        uint64_t *subset_miss_counter =
            canonical_return ? &rv64_jit_stats.direct_return_link_miss_count : &rv64_jit_stats.direct_jalr_link_miss_count;

        if (!emit_indirect_link_exit(w, regs, rd, link, completed_count + 1u, source_uses_data_state, pic_eligible, subset_taken_counter,
                                     subset_miss_counter, canonical_return ? RV64_JIT_INDIRECT_PIC_RETURN : RV64_JIT_INDIRECT_PIC_JALR))
        {
            return false;
        }
    }
    else if (!emit_mov_rcx_rax(w) || !emit_movabs_rax(w, link) || !jit_reg_write_rax(w, regs, rd) || !jit_reg_flush_all_dirty(w, regs) ||
             !emit_mov_rax_rcx(w) || !emit_store_rax_pc(w) || !emit_return_total_retired(w, completed_count + 1u))
    {
        return false;
    }
    ...
    if (!emit_interpreter_side_exit(w, &side_exit_regs, pc, completed_count, RV64_JIT_SIDE_EXIT_JALR_MISALIGNED))
    {
        return false;
    }
