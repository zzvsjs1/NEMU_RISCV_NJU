...
#if defined(__x86_64__) && defined(CONFIG_RV64_JIT) && defined(CONFIG_TARGET_NATIVE_ELF) && !defined(CONFIG_TRACE) && !defined(CONFIG_DIFFTEST) && \
...
#define RV64_JIT_PMEM_CHUNK_COUNT (((size_t)CONFIG_MSIZE + (size_t)RV64_JIT_SOURCE_CHUNK_SIZE - 1u) / (size_t)RV64_JIT_SOURCE_CHUNK_SIZE)
/*
...
#define RV64_JIT_DIRECT_LINK_MAX_MISS_PATCHES RV64_JIT_DIRECT_LINK_GUARD_COUNT
...
#define RV64_JIT_INDIRECT_JUMP_CACHE_INDEX_MASK (RV64_JIT_INDIRECT_JUMP_CACHE_ENTRIES - 1u)
#define RV64_JIT_INDIRECT_JUMP_CACHE_ENTRY_SHIFT 4u
#define RV64_JIT_INDIRECT_JUMP_CACHE_BYTE_MASK (RV64_JIT_INDIRECT_JUMP_CACHE_INDEX_MASK << RV64_JIT_CACHE_PC_SHIFT)
#define RV64_JIT_INDIRECT_JUMP_CACHE_OFFSET_SHIFT (RV64_JIT_INDIRECT_JUMP_CACHE_ENTRY_SHIFT - RV64_JIT_CACHE_PC_SHIFT)
#define RV64_JIT_INDIRECT_JUMP_CACHE_MAX_FIXUPS 1u
#define RV64_JIT_INDIRECT_JUMP_CACHE_LINE_SIZE 64u
#define RV64_JIT_INDIRECT_JUMP_CACHE_MAX_ALLOCATION ((RV64_JIT_INDIRECT_JUMP_CACHE_LINE_SIZE - 1u) + sizeof(rv64_jit_indirect_jump_cache_t))
/*
 ...
 */
_Static_assert(RV64_JIT_INDIRECT_PIC_WAYS == 2u, "RV64 JIT indirect PIC route ABI requires exactly two ways");
...
#define RV64_JIT_INDIRECT_PIC_MAX_ALLOCATION ((RV64_JIT_INDIRECT_PIC_LINE_SIZE - 1u) + sizeof(rv64_jit_indirect_pic_t))
/*
...
#define RV64_JIT_DIRECT_MMIO_MAX_CLASSIFIERS \
    ((RV64_JIT_DIRECT_MMIO_MAX_MAPS > RV64_JIT_DIRECT_MMIO_MAX_REGIONS) ? RV64_JIT_DIRECT_MMIO_MAX_MAPS : RV64_JIT_DIRECT_MMIO_MAX_REGIONS)
#define RV64_JIT_MMIO_ROUTE_MAX_FIXUPS (RV64_JIT_MMIO_ROUTE_MAX_SITES * (2u + 3u * RV64_JIT_DIRECT_MMIO_MAX_CLASSIFIERS))
#define RV64_JIT_MMIO_ROUTE_MAX_ALLOCATION ((RV64_JIT_MMIO_ROUTE_LINE_SIZE - 1u) + RV64_JIT_MMIO_ROUTE_LINE_SIZE)
/*
...
typedef struct
{
    rv64_jit_indirect_jump_cache_entry_t entries[RV64_JIT_INDIRECT_JUMP_CACHE_ENTRIES];
...
typedef struct
{
    uint8_t *address_immediates[RV64_JIT_INDIRECT_JUMP_CACHE_MAX_FIXUPS];
    rv64_jit_indirect_jump_cache_builder_state_t state;
} rv64_jit_indirect_jump_cache_builder_t;

_Static_assert((RV64_JIT_INDIRECT_JUMP_CACHE_ENTRIES & (RV64_JIT_INDIRECT_JUMP_CACHE_ENTRIES - 1u)) == 0,
               "RV64 JIT indirect jump cache entry count must be a power of two");
_Static_assert(RV64_JIT_INDIRECT_JUMP_CACHE_ENTRY_SHIFT >= RV64_JIT_CACHE_PC_SHIFT, "RV64 JIT indirect jump cache entries are too small");
_Static_assert(RV64_JIT_INDIRECT_JUMP_CACHE_BYTE_MASK <= INT8_MAX, "RV64 JIT indirect jump cache byte mask exceeds imm8");
_Static_assert(sizeof(rv64_jit_indirect_jump_cache_entry_t) == (1u << RV64_JIT_INDIRECT_JUMP_CACHE_ENTRY_SHIFT),
               "RV64 JIT indirect jump cache entry shift drifted");
_Static_assert(sizeof(rv64_jit_indirect_jump_cache_t) == 256u, "RV64 JIT indirect jump cache sidecar size drifted");
...
_Static_assert(offsetof(rv64_jit_indirect_pic_t, links) == RV64_JIT_INDIRECT_PIC_LINE_SIZE,
               "RV64 JIT indirect PIC hot header must occupy one cache line");
_Static_assert(sizeof(rv64_jit_indirect_pic_t) == 256u, "RV64 JIT indirect PIC sidecar size drifted");
...
typedef struct
{
    ...
    rv64_jit_mmio_route_fixup_t fixups[RV64_JIT_MMIO_ROUTE_MAX_FIXUPS];
} rv64_jit_mmio_route_builder_t;

_Static_assert(sizeof(rv64_jit_mmio_route_t) == 16u, "RV64 JIT MMIO route entries must be 16 bytes");
_Static_assert(RV64_JIT_MMIO_ROUTE_MAX_SITES * sizeof(rv64_jit_mmio_route_t) == RV64_JIT_MMIO_ROUTE_LINE_SIZE,
...
static inline rv64_jit_fp_exact_op_t rv64_jit_decode_fp_exact(uint32_t instr)
{
    ...
    case 0x70u: /* FMV.X.W and FCLASS.S share funct7. */
        ...
        return funct3 == 1 ? RV64_JIT_FP_EXACT_FCLASS_S : RV64_JIT_FP_EXACT_INVALID;
    case 0x78u: /* FMV.W.X. */
        return rs2 == 0 && funct3 == 0 ? RV64_JIT_FP_EXACT_FMV_W_X : RV64_JIT_FP_EXACT_INVALID;
    ...
    case 0x71u: /* FMV.X.D and FCLASS.D share funct7. */
        ...
        return funct3 == 1 ? RV64_JIT_FP_EXACT_FCLASS_D : RV64_JIT_FP_EXACT_INVALID;
    case 0x79u: /* FMV.D.X. */
        return rs2 == 0 && funct3 == 0 ? RV64_JIT_FP_EXACT_FMV_D_X : RV64_JIT_FP_EXACT_INVALID;
#endif
    default:
        return RV64_JIT_FP_EXACT_INVALID;
    }
...
static inline int64_t imm_b(uint32_t instr)
{
     ...
     */
    uint32_t imm = (bits(instr, 11, 8) << 1) | (bits(instr, 30, 25) << 5) | (bits(instr, 7, 7) << 11) | (bits(instr, 31, 31) << 12);
    return sext(imm, 13);
}
...
static inline int64_t imm_j(uint32_t instr)
{
     ...
     */
    uint32_t imm = (bits(instr, 30, 21) << 1) | (bits(instr, 20, 20) << 11) | (bits(instr, 19, 12) << 12) | (bits(instr, 31, 31) << 20);
    return sext(imm, 21);
}
 ...
 */
static inline uint32_t rv64_jit_cache_context_mix(word_t satp, uint32_t ifetch_state)
{
    return (uint32_t)(satp ^ (satp >> RV64_JIT_CACHE_SATP_MIX_SHIFT) ^ ifetch_state);
}

/* Return the existing cache index for one full guest fetch context. */
static inline uint32_t rv64_jit_cache_hash_context(vaddr_t pc, word_t satp, uint32_t ifetch_state)
{
    return (uint32_t)(((pc >> RV64_JIT_CACHE_PC_SHIFT) ^ rv64_jit_cache_context_mix(satp, ifetch_state)) & (RV64_JIT_CACHE_SIZE - 1u));
}
...
bool rv64_jit_finalise_mmio_routes(rv64_jit_writer_t *w, rv64_jit_mmio_route_builder_t *mmio_routes);
#ifdef CONFIG_RISCV_FPU
