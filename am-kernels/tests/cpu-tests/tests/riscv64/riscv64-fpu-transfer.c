...
asm(".section .text\n"
    ...
    FP_ASM_BITWISE_S(rv64_fp_sgnj_s, fsgnj.s) FP_ASM_BITWISE_S(rv64_fp_sgnjn_s, fsgnjn.s) FP_ASM_BITWISE_S(rv64_fp_sgnjx_s, fsgnjx.s)
        FP_ASM_BITWISE_D(rv64_fp_sgnj_d, fsgnj.d) FP_ASM_BITWISE_D(rv64_fp_sgnjn_d, fsgnjn.d) FP_ASM_BITWISE_D(rv64_fp_sgnjx_d, fsgnjx.d)
            FP_ASM_BITWISE_S(rv64_fp_min_s, fmin.s) FP_ASM_BITWISE_S(rv64_fp_max_s, fmax.s) FP_ASM_BITWISE_D(rv64_fp_min_d, fmin.d)
    ...
    FP_ASM_COMPARE_S(rv64_fp_eq_s, feq.s) FP_ASM_COMPARE_S(rv64_fp_lt_s, flt.s) FP_ASM_COMPARE_S(rv64_fp_le_s, fle.s)
        FP_ASM_COMPARE_D(rv64_fp_eq_d, feq.d) FP_ASM_COMPARE_D(rv64_fp_lt_d, flt.d) FP_ASM_COMPARE_D(rv64_fp_le_d, fle.d)
...
static const struct
{
    ...
    {UINT64_C(0xfff0000000000000), 1u << 0}, {UINT64_C(0xbff0000000000000), 1u << 1}, {UINT64_C(0x8000000000000001), 1u << 2},
    {UINT64_C(0x8000000000000000), 1u << 3}, {UINT64_C(0x0000000000000000), 1u << 4}, {UINT64_C(0x0000000000000001), 1u << 5},
    {UINT64_C(0x3ff0000000000000), 1u << 6}, {UINT64_C(0x7ff0000000000000), 1u << 7}, {UINT64_C(0x7ff0000000000001), 1u << 8},
...
static void test_sign_injection(void)
{
    ...
    check(rv64_fp_sgnj_s(s_payload, s_negative) == (UINT64_C(0xffffffff) << 32 | UINT32_C(0xffc12345)));
    check(rv64_fp_sgnjn_s(s_payload, s_negative) == (UINT64_C(0xffffffff) << 32 | s_payload));
    check(rv64_fp_sgnjx_s(UINT32_C(0xffc12345), s_negative) == (UINT64_C(0xffffffff) << 32 | s_payload));

    check(rv64_fp_sgnj_d(d_payload, d_negative) == UINT64_C(0xfff8123456789abc));
    check(rv64_fp_sgnjn_d(d_payload, d_negative) == d_payload);
    check(rv64_fp_sgnjx_d(UINT64_C(0xfff8123456789abc), d_negative) == d_payload);

    check(rv64_fp_sgnj_s_rd_rs2(s_payload, s_negative) == UINT64_C(0xffffffffffc12345));
    check(rv64_fp_sgnj_d_rd_rs2(d_payload, d_negative) == UINT64_C(0xfff8123456789abc));

    /*
     * Sign injection is bitwise and never signals, including for a signalling
     * NaN payload.
     */
    check(rv64_fp_sgnj_s(UINT32_C(0x7f800123), UINT32_C(0x3f800000)) == UINT64_C(0xffffffff7f800123));
...
{
    write_fflags(0);
    check(rv64_fp_min_s(UINT32_C(0x00000000), UINT32_C(0x80000000)) == UINT64_C(0xffffffff80000000));
    check(rv64_fp_max_s(UINT32_C(0x00000000), UINT32_C(0x80000000)) == UINT64_C(0xffffffff00000000));
    check(rv64_fp_min_d(UINT64_C(0x0000000000000000), UINT64_C(0x8000000000000000)) == UINT64_C(0x8000000000000000));
    check(rv64_fp_max_d(UINT64_C(0x0000000000000000), UINT64_C(0x8000000000000000)) == UINT64_C(0x0000000000000000));
    check(read_fflags() == 0);

    /* A signalling NaN returns the numeric operand and accrues NV. */
    check(rv64_fp_min_s(UINT32_C(0x7f800001), UINT32_C(0x3f800000)) == UINT64_C(0xffffffff3f800000));
...
static void test_compare_and_classify(void)
{
    ...
    check(rv64_fp_eq_d(UINT64_C(0x3ff0000000000000), UINT64_C(0x3ff0000000000000)) == 1);
    check(rv64_fp_lt_d(UINT64_C(0xbff0000000000000), UINT64_C(0x0000000000000000)) == 1);
    check(rv64_fp_le_d(UINT64_C(0x3ff0000000000000), UINT64_C(0x3ff0000000000000)) == 1);
    check(read_fflags() == 0);
    ...
    {
        check(rv64_fp_class_s(fclass_s_cases[i].bits) == fclass_s_cases[i].expected);
        check(read_fflags() == UINT64_C(0x1f));
    }

    for (unsigned i = 0; i < sizeof(fclass_d_cases) / sizeof(fclass_d_cases[0]); ++i)
    {
        check(rv64_fp_class_d(fclass_d_cases[i].bits) == fclass_d_cases[i].expected);
        check(read_fflags() == UINT64_C(0x1f));
    }
}
...
static void test_nan_boxing_and_raw_store_exception(void)
{
     ...
     */
    check(rv64_fp_malformed_s_add(UINT64_C(0x000000003f800000), UINT32_C(0x3f800000)) == UINT64_C(0xffffffff7fc00000));
     ...
     */
    check(rv64_fp_move_x_w(UINT32_C(0x7f800123)) == UINT64_C(0x000000007f800123));
    check(rv64_fp_move_x_w(UINT32_C(0xff800123)) == UINT64_C(0xffffffffff800123));
}
...
static void test_malformed_exact_inputs(void)
{
     ...
     */
    check(rv64_fp_malformed_s_sgnj(UINT64_C(0x000000003f800000), UINT64_C(0xffffffffbf800000)) == UINT64_C(0xffffffffffc00000));
    check(rv64_fp_malformed_s_sgnj(UINT64_C(0xffffffff3f800000), UINT64_C(0x00000000bf800000)) == UINT64_C(0xffffffff3f800000));
    check(rv64_fp_malformed_s_class(UINT64_C(0x000000003f800000)) == (1u << 9));
     ...
     */
    check(rv64_fp_malformed_move_x_w(UINT64_C(0x000000007fa12345)) == UINT64_C(0x000000007fa12345));
    check(rv64_fp_malformed_move_x_w(UINT64_C(0x0123456781234567)) == UINT64_C(0xffffffff81234567));
}
...
static void test_exact_fp_state_effects(void)
{
    ...
    check(rv64_fp_read_x_w_only() == UINT64_C(0xffffffff81234567));
    ...
    check((status & MSTATUS_FS_MASK) == MSTATUS_FS_DIRTY);
    check((status & MSTATUS_SD) != 0);
    check(rv64_fp_read_x_w_only() == UINT64_C(0x0000000001234567));
}
...
static void test_exact_ops_ignore_reserved_frm(void)
{
    ...
    {
        write_frm(frm);
        check(rv64_fp_sgnj_d(UINT64_C(0x3ff123456789abcd), UINT64_C(0xbff0000000000000)) == UINT64_C(0xbff123456789abcd));
        check(rv64_fp_class_d(UINT64_C(0x7ff0000000000001)) == (1u << 8));
    }
}
...
{
    write_fflags(UINT64_C(0x1f));
    check(rv64_fp_exact_stable_loop(UINT64_C(0x3ff0000000000000), UINT64_C(0xbff0000000000000), 64) == UINT64_C(0xbff0000000000000));
    check(read_fflags() == UINT64_C(0x1f));
}
