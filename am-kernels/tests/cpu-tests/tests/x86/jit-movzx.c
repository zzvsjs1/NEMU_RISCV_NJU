...
static const uint16_t words[16] __attribute__((aligned(4096))) = {
    0x0000u, 0x0001u, 0x007fu, 0x0080u, 0x00ffu, 0x0100u, 0x7fffu, 0x8000u, 0xffffu, 0x1234u, 0xabcdu, 0x55aau, 0xaa55u, 0x0f0fu, 0xf0f0u, 0x8001u,
};

static const uint8_t bytes[16] __attribute__((aligned(4096))) = {
    0x00u, 0x01u, 0x7fu, 0x80u, 0xffu, 0x12u, 0x34u, 0x56u, 0x78u, 0x9au, 0xbcu, 0xdeu, 0xf0u, 0x0fu, 0xa5u, 0x5au,
};
...
static uint32_t movzx_high_byte_sum(void)
{
    ...
    asm volatile("movl $0x11223344, %%eax\n\t"
                 ...
                 "movzbl %%bh, %%esi\n\t"
                 "movl %%esi, %[bh]\n\t"
                 : [ah] "=m"(ah), [ch] "=m"(ch), [dh] "=m"(dh), [bh] "=m"(bh)
                 :
