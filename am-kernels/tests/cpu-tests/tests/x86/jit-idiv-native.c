...
static int32_t do_idiv_ebx(int32_t dividend, int32_t divisor, int32_t *rem)
{
    ...
    asm volatile("movl %[divisor], %%ebx\n\t"
                 "cdq\n\t"
                 "idivl %%ebx"
                 : "=a"(quo), "=d"(out_rem)
                 : "a"(dividend), [divisor] "r"(divisor)
                 : "ebx", "cc");
...
int main(void)
{
    ...
    for (int32_t i = -2048; i < 2048; i++)
    {
        ...
        check(do_idiv_reg(dividend, reg_divisor, &rem) == dividend / reg_divisor);
