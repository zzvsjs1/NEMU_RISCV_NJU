...
int main() {
  ...
  asm volatile(
      ...
      "setc %[word_cf]\n\t"
      "sets %[word_sf]"
      : [byte_shr] "+m"(shift_byte_mem),
        [word_sar] "+m"(shift_word_mem),
        [byte_cf] "=qm"(byte_cf),
        [byte_zf] "=qm"(byte_zf),
        [word_cf] "=qm"(word_cf),
        [word_sf] "=qm"(word_sf)
  ...
  asm volatile(
      ...
      "setz %[byte_zf]\n\t"
      "movb %%al, %[byte_out]"
      : [word_out] "=m"(word_reg_shr),
        [word_cf] "=qm"(word_reg_cf),
        [word_zf] "=qm"(word_reg_zf),
        [byte_out] "=m"(byte_reg_shr),
        [byte_cf] "=qm"(byte_reg_cf),
        [byte_zf] "=qm"(byte_reg_zf)
      :
      : "eax", "memory", "cc");
