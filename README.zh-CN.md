...
- `nemu`：模拟器核心、RISC-V32/RISC-V64 执行器、设备模型和可选 RV32 JIT。
...
当前 `master` 分支是 RISC-V32 JIT 性能改进版本。`riscv64` 分支是当前
RV64 NEMU/Nanos-lite bring-up 分支。旧分支会保留下来作为对比点，方便比较
原始 baseline、磁盘/ONScripter 改动、非 JIT 性能改动、JIT 版本和 RV64
bring-up 工作之间的行为与性能差异。
...
| `riscv64` | 当前 RISC-V64 NEMU/Nanos-lite bring-up 分支 |
...
在当前 RV64 分支上，先配置并构建 NEMU，再重建 Nanos-lite 磁盘镜像，并用
`riscv64-nemu` 运行：
...
bring-up 调试。RV64 路径目前没有 RV32 x86-64 JIT；它使用解释器执行。

## JIT 配置

RV32 JIT 分支在 `nemu/menuconfig` 的 `RISC-V32 JIT` 菜单中加入了 JIT 选项。这个
...
`NEMU_DISABLE_JIT=1` 只会在 `CONFIG_RV32_JIT` 构建中关闭 JIT。它不是通用的
“纯解释器”开关。如果二进制是用 `CONFIG_RV32_FAST_EXEC` 构建的，CPU loop 仍然
可以走 fast executor。要保证 RV32 纯解释器运行，需要重新构建 NEMU，并把
acceleration mode 设为 `CONFIG_RV32_ACCEL_NONE`。
...
如果和 RV32 JIT 行比较，RV64 仍然慢很多：大约是 RV32 JIT MicroBench
throughput 的 `1/18.5`，也是 RV32 JITBench throughput 的 `1/26.2`。这主要说明
RV64 还没有移植加速路径。
