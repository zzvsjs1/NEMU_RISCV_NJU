...
void iNESGI(GI h)
{ //bbit edited: removed static keyword
    switch (h)
    {
    case GI_RESETSAVE:
        break;
...
int iNESLoad(const char *name, FCEUFILE *fp, int OverwriteVidMode)
{
    ...
    FCEU_printf(" Mirroring: %s\n", Mirroring == 2 ? "None (Four-screen)" : Mirroring ? "Vertical"
                                                                                      : "Horizontal");
    ...
    while (tmp->init)
    {
        if (num == tmp->number)
        {
            ...
            if (!VROM_size)
            {
                ...
                {
                    switch (num)
                    { // FIXME, mapper or game data base with the board parameters and ROM/RAM sizes
                    case 13:
                        CHRRAMSize = 16 * 1024;
                        break;
                    case 6:
