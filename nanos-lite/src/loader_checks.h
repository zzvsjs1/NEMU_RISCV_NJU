 ...
 */
static inline bool nanos_loader_checked_add_uintptr(uintptr_t base, uintptr_t increment, uintptr_t *result)
{
 ...
 */
static inline bool nanos_loader_load_range_fits(uintptr_t user_start, uintptr_t user_end, size_t stack_bytes, uintptr_t segment_start,
                                                uintptr_t segment_bytes, uintptr_t *segment_end)
{
    uintptr_t end;
    uintptr_t stack_base;

    if (user_end < user_start || stack_bytes > user_end - user_start || !nanos_loader_checked_add_uintptr(segment_start, segment_bytes, &end))
    {
        return false;
    }
 ...
 */
static inline bool nanos_loader_stack_layout_fits(uintptr_t stack_base, uintptr_t stack_end, size_t string_bytes, size_t pointer_words,
                                                  uintptr_t *initial_sp)
{
    ...
    if (stack_end < stack_base || string_bytes > stack_end - stack_base || pointer_words > (size_t)-1 / sizeof(uintptr_t))
    {
        return false;
    }
