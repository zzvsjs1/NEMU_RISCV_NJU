...
static void overwrite_fat_entry_raw(const Fat32Volume *vol, uint32_t cluster, uint32_t value)
{
    FILE *image = fopen("fat32-read-work/ramdisk.img", "rb+");
    assert(image != 0);
    assert(fseek(image, (long)((size_t)vol->reserved_sector_count * 512u + (size_t)cluster * 4u), SEEK_SET) == 0);
...
static void test_reads_fat_chain_from_image(void)
{
    ...
    assert(disk_read(root_sector_data, (size_t)root_sector * 512u, sizeof(root_sector_data)) == sizeof(root_sector_data));
