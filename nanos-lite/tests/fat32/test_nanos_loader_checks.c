...
static int test_load_range_rejects_kernel_and_stack_addresses(void)
{
    ...
    CHECK(nanos_loader_load_range_fits(user_start, user_end, stack_bytes,
                                       0x40001000u, 0x2000u, &segment_end));
    CHECK(segment_end == 0x40003000u);
    CHECK(nanos_loader_load_range_fits(user_start, user_end, stack_bytes,
                                       user_end - stack_bytes - PAGE_SIZE,
                                       PAGE_SIZE, &segment_end));
    CHECK(segment_end == user_end - stack_bytes);

    CHECK(!nanos_loader_load_range_fits(user_start, user_end, stack_bytes,
                                        0x80000000u, PAGE_SIZE, &segment_end));
    CHECK(!nanos_loader_load_range_fits(user_start, user_end, stack_bytes,
                                        user_end - stack_bytes - 1u, 2u, &segment_end));
    CHECK(!nanos_loader_load_range_fits(user_start, user_end, stack_bytes,
                                        UINTPTR_MAX - 7u, 16u, &segment_end));
...
static int test_stack_layout_rejects_underflow(void)
{
    ...
    CHECK(nanos_loader_stack_layout_fits(stack_base, stack_end,
                                         stack_bytes - pointer_bytes,
                                         pointer_words, &initial_sp));
    CHECK(initial_sp == stack_base);

    CHECK(!nanos_loader_stack_layout_fits(stack_base, stack_end,
                                          stack_bytes, pointer_words,
                                          &initial_sp));
    CHECK(!nanos_loader_stack_layout_fits(stack_base, stack_end, 0,
                                          stack_bytes / sizeof(uintptr_t) + 1u,
                                          &initial_sp));
...
int main(void)
{
    if (test_load_range_rejects_kernel_and_stack_addresses() != 0 ||
        test_stack_layout_rejects_underflow() != 0)
    {
        return 1;
    }
