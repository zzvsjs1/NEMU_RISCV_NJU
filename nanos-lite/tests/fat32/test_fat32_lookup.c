    ...
    for (uint32_t sector_index = 0; sector_index < vol->sectors_per_cluster; sector_index++) {
      ...
      for (size_t i = 0; i < FAT32_DIR_ENTRIES_PER_SECTOR; i++) {
        ...
        if (memcmp(entry, short_name, 11) == 0) {
          *entry_offset = base + (uint64_t)sector_index * FAT32_SECTOR_SIZE
                          ...
                          uint64_t *short_offset, uint8_t short_entry[FAT32_DIR_ENTRY_SIZE])
{
  ...
  for (uint32_t links = 0; links < vol->cluster_count; links++) {
    ...
    for (uint32_t sector_index = 0; sector_index < vol->sectors_per_cluster; sector_index++) {
      ...
      for (size_t i = 0; i < FAT32_DIR_ENTRIES_PER_SECTOR; i++) {
        const uint8_t *entry = &sector[i * FAT32_DIR_ENTRY_SIZE];
        uint64_t offset = base + (uint64_t)sector_index * FAT32_SECTOR_SIZE
            + i * FAT32_DIR_ENTRY_SIZE;
        ...
        if (entry[11] == FAT32_ATTR_LONG_NAME) {
          ...
          if (sequence > 0 && sequence < 20
              && fat32_lfn_entry_to_ascii(&lfn, pieces[sequence], sizeof(pieces[sequence])) >= 0) {
            present[sequence] = 1;
          }
          continue;
        }
...
{
  uint8_t raw[4];
  uint64_t offset = (uint64_t)vol->reserved_sector_count * FAT32_SECTOR_SIZE
      + (uint64_t)cluster * 4u;
